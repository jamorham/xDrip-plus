...
public class ListenerService extends WearableListenerService implements GoogleApiClient.ConnectionCallbacks,
    ...
    public class DataRequester extends AsyncTask<Void, Void, Void> {
        ...
        DataRequester(Context context, String thispath, byte[] thispayload) {
           ...
           UserError.Log.d(TAG, "DataRequester DataRequester: " + thispath + " lastRequest:" + JoH.dateTimeText(lastRequest));
        }
        ...
        protected Void doInBackground(Void... params) {
            ...
            try {
               ...
               UserError.Log.d(TAG, "doInBackground enter enable_wearG5=" + enable_wearG5 + " force_wearG5=" + force_wearG5 + " node_wearG5=" + node_wearG5);//KS

                if (isCancelled()) {
                   UserError.Log.d(TAG, "doInBackground CANCELLED programmatically");
                    return null;
                }
                ...
                if (googleApiClient != null) {
                    ...
                    while (!googleApiClient.isConnected() && JoH.tsl() < timeout) {
                       ...
                       UserError.Log.d(TAG, "Sleeping for connect, remaining: " + JoH.niceTimeScalar(JoH.msTill(timeout)));
                ...
                if ((googleApiClient != null) && (googleApiClient.isConnected())) {
                    if (!path.equals(ACTION_RESEND) || (System.currentTimeMillis() - lastRequest > 20 * 1000)) { // enforce 20-second debounce period
                        ...
                        if (!capabilityResult.getStatus().isSuccess()) {
                           UserError.Log.e(TAG, "doInBackground Failed to get capabilities, status: " + capabilityResult.getStatus().getStatusMessage());
                            return null;
                        }
                       ...
                       UserError.Log.d(TAG, "doInBackground connected.  CapabilityApi.GetCapabilityResult mPhoneNodeID=" + (phoneNode != null ? phoneNode.getId() : "") + " count=" + count + " localnode=" + localnode);//KS
                        if (count > 0) {
                            if (enable_wearG5) {
                                if (force_wearG5) {
                                    startBtService();
                            ...
                            for (Node node : capabilityInfo.getNodes()) {
                                ...
                                if (bInitPrefs) {
                                   UserError.Log.d(TAG, "doInBackground Request Phone's Preferences: WEARABLE_INITPREFS_PATH");
                               ...
                               UserError.Log.d(TAG, "doInBackground path: " + path);
                                switch (path) {
                                    ...
                                    case WEARABLE_RESEND_PATH:
                                       UserError.Log.d(TAG, "doInBackground WEARABLE_RESEND_PATH");
                                    ...
                                    default://SYNC_ALL_DATA
                                        ...
                                        if (path.startsWith(WEARABLE_REQUEST_APK)) {
                                           ...
                                           UserError.Log.d(TAG, "doInBackground SYNC_ALL_DATA - or unknown path: " + path);
                                            ...
                                            if (enable_wearG5) {//KS
                                                ...
                                                if (datamap == null) {
                                                   ...
                                                   UserError.Log.d(TAG, "Sending transmitter data: " + datamap.size());
                            ...
                            if (enable_wearG5) {//KS
                               UserError.Log.d(TAG, "doInBackground connected but getConnectedNodes returns 0.  Start BT service");
                                startBtService();
                            }
                        }
                    } else {
                       UserError.Log.d(TAG, "Debounce limit hit - not sending");
                    }
                } else {
                   UserError.Log.d(TAG, "Not connected for sending: api " + ((googleApiClient == null) ? "is NULL!" : "not null"));
                    if (googleApiClient != null) {
                        googleApiClient.connect();
                    } else {
                        googleApiConnect();
        ...
        private boolean runBenchmarkTest(Node node, String pathdesc, String path, byte[] payload, boolean bDuplicateTest) {
            if (mAsyncBenchmarkTester != null) {
               UserError.Log.d(TAG, "Benchmark: runAsyncBenchmarkTester mAsyncBenchmarkTester != null lastRequest:" + JoH.dateTimeText(lastRequest));
                if (mAsyncBenchmarkTester.getStatus() != AsyncTask.Status.FINISHED) {
                   UserError.Log.d(TAG, "Benchmark: mAsyncBenchmarkTester let process complete, do not start new process.");
                    //mAsyncBenchmarkTester.cancel(true);
                }
                //mAsyncBenchmarkTester = null;
            } else {
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
                   UserError.Log.d(TAG, "Benchmark: runAsyncBenchmarkTester SDK < M call execute lastRequest:" + JoH.dateTimeText(lastRequest));
                   ...
                   UserError.Log.d(TAG, "Benchmark: runAsyncBenchmarkTester SDK >= M call executeOnExecutor lastRequest:" + JoH.dateTimeText(lastRequest));
        ...
        private void sendMessagePayload(Node node, String pathdesc, final String path, byte[] payload) {
           UserError.Log.d(TAG, "Benchmark: doInBackground sendMessagePayload " + pathdesc + "=" + path + " nodeID=" + node.getId() + " nodeName=" + node.getDisplayName() + ((payload != null) ? (" payload.length=" + payload.length) : ""));
            ...
            result.setResultCallback(sendMessageResult -> {
                if (!sendMessageResult.getStatus().isSuccess()) {
                   UserError.Log.e(TAG, "sendMessagePayload ERROR: failed to send request " + path + " Status=" + sendMessageResult.getStatus().getStatusMessage());
                } else {
                   UserError.Log.d(TAG, "sendMessagePayload Sent request " + node.getDisplayName() + " " + path + " Status=: " + sendMessageResult.getStatus().getStatusMessage());
                }
            });
            ...
            if (bBenchmarkRandom) {
               ...
               UserError.Log.i(TAG, "Benchmark: DONE!");
            }
            //******************************************************************************
        }
        ...
        protected void onCancelled() {
           UserError.Log.d(TAG, "DataRequester AsyncTask doInBackground was cancelled");
        }
    }
    ...
    public class AsyncBenchmarkTester extends AsyncTask<Void, Void, Void> {
        ...
        AsyncBenchmarkTester(Context context, Node thisnode, String thispathdesc, String thispath, byte[] thispayload, boolean thisbduplicate) {
           ...
           UserError.Log.d(TAG, "AsyncBenchmarkTester: " + thispath);
        }
        ...
        protected Void doInBackground(Void... params) {
            if (payload != null) {
                ...
                //Test to be sure message can be delivered of this size.  It should show up in the phoneUserError.Log.
                /*
               UserError.Log.i(TAG, "Benchmark: " + pathdesc + "_COMPRESS" + " sendMessage Async check msg is delivered with len=" + comprPayload.length * 50);
                ...
                result.setResultCallback(new ResultCallback<MessageApi.SendMessageResult>() {
                    ...
                    public void onResult(MessageApi.SendMessageResult sendMessageResult) {
                        if (!sendMessageResult.getStatus().isSuccess()) {
                           UserError.Log.e(TAG, "Benchmark: ERROR: failed to ASYNC sendMessage " + pathdesc + "_COMPRESS" + " Status=" + sendMessageResult.getStatus().getStatusMessage());
                        } else {
                           UserError.Log.i(TAG, "Benchmark: Sent request " + pathdesc + "_COMPRESS" + " Status=" + sendMessageResult.getStatus().getStatusMessage());
                        }
                    }
                });
                */
        ...
        private void sendMessagePayloadTest(Node node, String pathdesc, String path, byte[] payload) {
           ...
           UserError.Log.i(TAG, "Benchmark: " + pathdesc + " Send 50x len=" + payload.length * 50 + " " + (ts() - benchmark_time) + " ms");
        }
    ...
    private synchronized DataMap getWearBgReadingData(int count, long last_send_time, int min_count) {
       ...
       UserError.Log.d(TAG, "getWearBgReadingData last_send_time:" + JoH.dateTimeText(last_send_time));

        BgReading last_bg = BgReading.last();
        if (last_bg != null) {
           UserError.Log.d(TAG, "getWearBgReadingData last_bg.timestamp:" + JoH.dateTimeText(last_bg.timestamp));
        }

        if (last_bg != null && last_send_time <= last_bg.timestamp) {//startTime
            long last_send_success = last_send_time;
           UserError.Log.d(TAG, "getWearBgData last_send_time < last_bg.timestamp:" + JoH.dateTimeText(last_bg.timestamp));
            ...
            if (!graph_bgs.isEmpty() && graph_bgs.size() > min_count) {
               ...
               UserError.Log.i(TAG, "getWearBgReadingData SYNCED BGs up to " + JoH.dateTimeText(last_send_success) + " count = " + graph_bgs.size());
                return entries;
            } else
               UserError.Log.i(TAG, "getWearBgReading SYNCED BGs up to " + JoH.dateTimeText(last_send_success) + " count = 0");
        }
        return null;
    }
    ...
    private synchronized DataMap getWearTransmitterData(int count, long last_send_time, int min_count) {//KS
       ...
       UserError.Log.d(TAG, "getWearTransmitterData last_send_time:" + JoH.dateTimeText(last_send_time));

        TransmitterData last_bg = TransmitterData.last();
        if (last_bg != null) {
           UserError.Log.d(TAG, "getWearTransmitterData last_bg.timestamp:" + JoH.dateTimeText(last_bg.timestamp));
        }

        if (last_bg != null && last_send_time <= last_bg.timestamp) {//startTime
            long last_send_success = last_send_time;
           UserError.Log.d(TAG, "getWearTransmitterData last_send_time < last_bg.timestamp:" + JoH.dateTimeText(last_bg.timestamp));
            ...
            if (!graph_bgs.isEmpty() && graph_bgs.size() > min_count) {
                ...
                entries.putLong("time", new Date().getTime()); // MOST IMPORTANT LINE FOR TIMESTAMP
                entries.putDataMapArrayList("entries", dataMaps);
               UserError.Log.i(TAG, "getWearTransmitterData SYNCED BGs up to " + JoH.dateTimeText(last_send_success) + " count = " + graph_bgs.size());
                return entries;
            } else
               UserError.Log.i(TAG, "getWearTransmitterData SYNCED BGs up to " + JoH.dateTimeText(last_send_success) + " count = 0");
        }
        return null;
    }
    ...
    private synchronized DataMap getWearLogData(int count, long last_send_time, int min_count, long syncLogsRequested) {
       ...
       UserError.Log.d(TAG, "getWearLogData last_send_time:" + JoH.dateTimeText(last_send_time) + " max count=" + count + " min_count=" + min_count + " syncLogsRequested=" + syncLogsRequested);

        UserError last_log = UserError.last();
        if (last_log != null) {
           UserError.Log.d(TAG, "getWearLogData last_log.timestamp:" + JoH.dateTimeText((long) last_log.timestamp));
        }

        if (last_log != null && last_send_time <= last_log.timestamp) {//startTime
            long last_send_success = last_send_time;
           UserError.Log.d(TAG, "getWearLogData last_send_time < last_bg.timestamp:" + JoH.dateTimeText((long) last_log.timestamp));
            ...
            if (!logs.isEmpty() && logs.size() > min_count) {
                ...
                for (UserError log : logs) {
                    ...
                    //Log.d(TAG, "getWearLogData set last_send_sucess:" + JoH.dateTimeText(last_send_sucess) + " Log:" +UserError.Log.toString());
                }
                entries.putLong("time", new Date().getTime()); // MOST IMPORTANT LINE FOR TIMESTAMP
                entries.putLong("syncLogsRequested", syncLogsRequested);
                entries.putDataMapArrayList("entries", dataMaps);
               UserError.Log.i(TAG, "getWearLogData SYNCED logs up to " + JoH.dateTimeText(last_send_success) + " count = " + logs.size() + " syncLogsRequested=" + syncLogsRequested);
                return entries;
            } else
               UserError.Log.i(TAG, "getWearLogData SYNCED logs up to " + JoH.dateTimeText(last_send_success) + " count = 0" + " syncLogsRequested=" + syncLogsRequested);
        }
        return null;
    }
    ...
    private synchronized DataMap getWearStepSensorData(int count, long last_send_time, int min_count) {//final int sensorType, final int accuracy, final long timestamp, final float[] values) {
       ...
       UserError.Log.d(TAG, "getWearStepSensorData last_send_time:" + JoH.dateTimeText(last_send_time));

        PebbleMovement last_log = PebbleMovement.last();
        if (last_log != null) {
           UserError.Log.d(TAG, "getWearStepSensorData last_log.timestamp:" + JoH.dateTimeText((long) last_log.timestamp));
        }
        else {
           UserError.Log.d(TAG, "getWearStepSensorData PebbleMovement.last() = null:");
        }

        if (last_log != null && last_send_time <= last_log.timestamp) {//startTime
            long last_send_success = last_send_time;
           UserError.Log.d(TAG, "getWearStepSensorData last_send_time < last_bg.timestamp:" + JoH.dateTimeText((long) last_log.timestamp));
            ...
            if (!logs.isEmpty() && logs.size() > min_count) {
                ...
                for (PebbleMovement log : logs) {
                    dataMaps.add(dataMap(log));
                    last_send_success = (long)log.timestamp;
                    //Log.d(TAG, "getWearStepSensorData set last_send_sucess:" + JoH.dateTimeText(last_send_success) + " pw.metric: " +UserError.Log.metric + " pw.timestamp: " + JoH.dateTimeText(log.timestamp));
                }
                entries.putLong("time", new Date().getTime()); // MOST IMPORTANT LINE FOR TIMESTAMP
                entries.putDataMapArrayList("entries", dataMaps);
                //Log.d(TAG, "getWearStepSensorData  entries:" + entries);
               UserError.Log.i(TAG, "getWearStepSensorData SYNCED steps up to " + JoH.dateTimeText(last_send_success) + " count = " + logs.size());
                return entries;
            } else
               UserError.Log.i(TAG, "getWearStepSensorData SYNCED steps up to " + JoH.dateTimeText(last_send_success) + " count = 0");
        }
        return null;
    }
    ...
    private synchronized DataMap getWearTreatmentsData(int count, long last_send_time, int min_count) {
       ...
       UserError.Log.d(TAG, "getWearTreatmentsData last_send_time:" + JoH.dateTimeText(last_send_time) + " max count=" + count + " min_count=" + min_count);

        Treatments last_log = Treatments.lastSystime();
        if (last_log != null) {
           UserError.Log.d(TAG, "getWearTreatmentsData last systimestamp: " + last_log.systimestamp + " " + JoH.dateTimeText((long) last_log.systimestamp));
        }

        if (last_log != null && last_log.systimestamp > 0 && last_send_time <= last_log.systimestamp) {//startTime
            long last_send_success = last_send_time;
           UserError.Log.d(TAG, "getWearTreatmentsData last_send_time < last_log.timestamp:" + JoH.dateTimeText((long) last_log.systimestamp));
            ...
            if (!logs.isEmpty() && logs.size() > min_count) {
                ...
                for (Treatments log : logs) {
                    ...
                    //Log.d(TAG, "getWearTreatmentsData set last_send_sucess:" + JoH.dateTimeText(last_send_sucess) + " Log:" +UserError.Log.toString());
                }
                entries.putLong("time", new Date().getTime()); // MOST IMPORTANT LINE FOR TIMESTAMP
                entries.putDataMapArrayList("entries", dataMaps);
               UserError.Log.i(TAG, "getWearTreatmentsData SYNCED treatments up to " + JoH.dateTimeText(last_send_success) + " count = " + logs.size());
                return entries;
            } else
               UserError.Log.i(TAG, "getWearTreatmentsData SYNCED treatments up to " + JoH.dateTimeText(last_send_success) + " count = 0");
        }
        return null;
    }
    ...
    private void sendPrefSettings() {//KS
       ...
       UserError.Log.d(TAG, "sendPrefSettings enter");
        forceGoogleApiConnect();
        ...
        if (localnode == null || (localnode != null && localnode.isEmpty())) setLocalNodeName();
       UserError.Log.d(TAG, "sendPrefSettings enable_wearG5: " + enable_wearG5 + " force_wearG5:" + force_wearG5 + " node_wearG5:" + node_wearG5 + " localnode:" + localnode + " dex_txid:" + dex_txid + " show_wear_treatments:" + show_wear_treatments);
        ...
        if (!node_wearG5.equals(dataMap.getString("node_wearG5", ""))) {
           UserError.Log.d(TAG, "sendPrefSettings save to SharedPreferences - node_wearG5:" + dataMap.getString("node_wearG5", ""));
    ...
    private DataMap dataMap(Treatments log) {
        DataMap dataMap = new DataMap();
        //String json =UserError.Log.toS();
        //Log.d(TAG, "dataMap PebbleMovement GSON: " + json);
        //dataMap.putString("entry", json);
        //dataMap.putLong("timestamp",UserError.Log.timestamp);
        String notes = log.uuid + " uuid " + log.notes;
       UserError.Log.d(TAG, "dataMap Treatments notes:" + notes);
    ...
    private DataMap dataMap(TransmitterData bg) {//KS
        DataMap dataMap = new DataMap();
        String json = bg.toS();
       UserError.Log.d(TAG, "dataMap BG GSON: " + json);
    ...
    private synchronized void sendData(String path, byte[] payload) {
        ...
        if (mDataRequester != null) {
           UserError.Log.d(TAG, "sendData DataRequester != null lastRequest:" + JoH.dateTimeText(lastRequest));
            if (mDataRequester.getStatus() != AsyncTask.Status.FINISHED) {
               UserError.Log.d(TAG, "sendData Should be canceled?  Let run 'til finished.");
                //mDataRequester.cancel(true);
            }
            //mDataRequester = null;
        }
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
           UserError.Log.d(TAG, "sendData SDK < M call execute lastRequest:" + JoH.dateTimeText(lastRequest));
            mDataRequester = (DataRequester) new DataRequester(this, path, payload).execute();
        } else {
           UserError.Log.d(TAG, "sendData SDK >= M call executeOnExecutor lastRequest:" + JoH.dateTimeText(lastRequest));
            // TODO xdrip executor
    ...
    private void forceGoogleApiConnect() {
        if ((googleApiClient != null && !googleApiClient.isConnected() && !googleApiClient.isConnecting()) || googleApiClient == null) {
            try {
               UserError.Log.d(TAG, "forceGoogleApiConnect: forcing google api reconnection");
                googleApiConnect();
                Thread.sleep(2000);
    ...
    public void onPeerConnected(Node peer) {//Deprecated with BIND_LISTENER
       ...
       UserError.Log.d(TAG, "onPeerConnected peer name & ID: " + name + "|" + id);
    ...
    public void onPeerDisconnected(Node peer) {//Deprecated with BIND_LISTENER
        ...
        String id = peer.getId();
        String name = peer.getDisplayName();
       UserError.Log.d(TAG, "onPeerDisconnected peer name & ID: " + name + "|" + id);
        mPrefs = PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
    ...
    public void onCreate() {
       UserError.Log.d(TAG, "onCreate entered");
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
       UserError.Log.d(TAG, "onStartCommand entered");
    ...
    final private SharedPreferences.OnSharedPreferenceChangeListener prefListener = (prefs, key) -> {
       ...
       UserError.Log.d(TAG, "OnSharedPreferenceChangeListener entered key=" + ((key != null && !key.isEmpty()) ? key : ""));
        if(key.compareTo("enable_wearG5") == 0 || key.compareTo("force_wearG5") == 0 || key.compareTo("node_wearG5") == 0) {
           UserError.Log.i(TAG, "OnSharedPreferenceChangeListener sendPrefSettings and processConnect for key=" + key);
            sendPrefSettings();
            processConnect();
        }
           ...
           UserError.Log.d(TAG, "OnSharedPreferenceChangeListener sendPrefSettings for key=" + key);
            sendPrefSettings();
        }
        ...
        /*else if (key.compareTo("show_wear_treatments") == 0) {
           UserError.Log.d(TAG, "OnSharedPreferenceChangeListener sendPrefSettings for key=" + key);
            ...
            if (prefs.getBoolean("overrideLocale", false)) {
               UserError.Log.d(TAG, "overrideLocale true; Request phone locale");
            ...
            if (restartCollectorPrefs.contains(key)) {
               UserError.Log.d(TAG, "OnSharedPreferenceChangeListener restartCollectorPrefs requires collector restart key=" + key);
                stopBtService();
            }
            processConnect();
        }
    ...
    public void onDataChanged(DataEventBuffer dataEvents) {
        ...
        for (DataEvent event : dataEvents) {
            if (event.getType() == DataEvent.TYPE_CHANGED) {
               ...
               UserError.Log.d(TAG, "onDataChanged top path=" + path + " DataMap=" + dataMap);
                switch (path) {
                    case OPEN_SETTINGS:
                        //TODO: OpenSettings
                    ...
                    case NEW_STATUS_PATH:
                       ...
                       UserError.Log.d(TAG, "onDataChanged NEW_STATUS_PATH=" + path + " showExternalStatus=" + showExternalStatus);
                    ...
                    case WEARABLE_TOAST_LOCAL_NOTIFICATON:
                       ...
                       UserError.Log.d(TAG, "onDataChanged WEARABLE_TOAST_LOCAL_NOTIFICATON=" + path);
                        break;
                    case WEARABLE_DATA_PATH:
                        dataMap = DataMapItem.fromDataItem(event.getDataItem()).getDataMap();
                       UserError.Log.d(TAG, "onDataChanged WEARABLE_DATA_PATH=" + path);
                        if (resetDataToLatest(dataMap, getApplicationContext())) {
                           UserError.Log.d(TAG, "onDataChanged dataMap reset to watch BgReading.Last()");
                        }
                    ...
                    case WEARABLE_SNOOZE_ALERT:
                       UserError.Log.d(TAG, "onDataChanged WEARABLE_SNOOZE_ALERT=" + path);
                        dataMap = DataMapItem.fromDataItem(event.getDataItem()).getDataMap();
                        if (dataMap != null) {
                           ...
                           UserError.Log.d(TAG, "Received wearable: snooze payload: " + snooze);
                    ...
                    case SYNC_DB_PATH: {//KS  || path.equals(RESET_DB_PATH)
                       UserError.Log.d(TAG, "onDataChanged SYNC_DB_PATH=" + path);
                       ...
                       UserError.Log.d(TAG, "onDataChanged SYNC_DB_PATH delete BgReading and Calibration < retainFrom=" + JoH.dateTimeText(retainFrom));
                        BgReading.cleanup(retainFrom);
                        Calibration.cleanup(retainFrom);
                       UserError.Log.d(TAG, "onDataChanged SYNC_DB_PATH delete UserError < last_send_previous_log=" + JoH.dateTimeText(last_send_previous_log));
                        UserError.cleanup(last_send_previous_log);
                       UserError.Log.d(TAG, "onDataChanged SYNC_DB_PATH delete TransmitterData < last_send_previous=" + JoH.dateTimeText(last_send_previous));
                        TransmitterData.cleanup(last_send_previous);
                       UserError.Log.d(TAG, "onDataChanged SYNC_DB_PATH delete PebbleMovement < last_send_previous=" + JoH.dateTimeText(last_send_previous_step_sensor));
                    ...
                    case RESET_DB_PATH: {//KS
                       UserError.Log.d(TAG, "onDataChanged RESET_DB_PATH=" + path);
                   ...
                   /* Sensor.DeleteAndInitDb(getApplicationContext());
                    ...
                    if (isSafeToDeleteDB()) {
                        ...
                        doDeleteDB = true;
                       UserError.Log.d(TAG, "onDataChanged RESET_DB_PATH=" + path + " Unable to delete wear DB; wear data needs syncing.");
                    }*/
                        JoH.releaseWakeLock(wl);
                        break;
                    }
                    case SYNC_LOGS_PATH:
                       UserError.Log.d(TAG, "onDataChanged SYNC_LOGS_PATH=" + path);
                        //requestData();
                        dataMap = DataMapItem.fromDataItem(event.getDataItem()).getDataMap();
                        if (dataMap != null) {
                           ...
                           UserError.Log.d(TAG, "onDataChanged Received SYNC_LOGS_PATH request syncLogsRequested=" + syncLogsRequested);
                            dataMap = getWearLogData(send_log_count, last_send_previous_log, (send_log_count / 3), syncLogsRequested);
                            if (dataMap != null) {
                               UserError.Log.i(TAG, "onDataChanged SYNC_LOGS_PATH Request from last log processed " + JoH.dateTimeText(last_send_previous_log));
                                sendData(SYNC_LOGS_PATH, dataMap.toByteArray());
                            } else {
                               UserError.Log.d(TAG, "SYNC_LOGS_PATH received! No outstanding logs! ACTION_SYNC_LOGS request completed!! Ongoing requests syncLogsRequested=" + syncLogsRequested);
                    ...
                    case CLEAR_LOGS_PATH:
                       UserError.Log.d(TAG, "onDataChanged CLEAR_LOGS_PATH=" + path);
                        try {
                            UserError.cleanup();
                        } catch (Exception e) {
                           UserError.Log.e(TAG, "onDataChanged CLEAR_LOGS_PATH exception on UserError ", e);
                        }
                        break;
                    case CLEAR_TREATMENTS_PATH:
                       UserError.Log.d(TAG, "onDataChanged CLEAR_TREATMENTS_PATH=" + path + " last_send_previous_treatments=" + JoH.dateTimeText(last_send_previous_treatments));
                        try {
                            Treatments.cleanup(last_send_previous_treatments);
                        } catch (Exception e) {
                           UserError.Log.e(TAG, "onDataChanged CLEAR_TREATMENTS_PATH exception on Treatments ", e);
                        }
                        break;
                    case START_COLLECTOR_PATH:
                       UserError.Log.d(TAG, "onDataChanged START_COLLECTOR_PATH=" + path);
                        stopBtService();
                        if (processConnect()) {
                    ...
                    case STATUS_COLLECTOR_PATH:
                       UserError.Log.d(TAG, "onDataChanged path=" + path);
                        dataMap = DataMapItem.fromDataItem(event.getDataItem()).getDataMap();
                    ...
                    case WEARABLE_SENSOR_DATA_PATH: //KS
                        dataMap = DataMapItem.fromDataItem(event.getDataItem()).getDataMap();
                       UserError.Log.d(TAG, "onDataChanged path=" + path + " DataMap=" + dataMap);
                    ...
                    case DATA_ITEM_RECEIVED_PATH: //KS
                       ...
                       UserError.Log.d(TAG, "onDataChanged path=" + path + " DataMap=" + dataMap);
                        ...
                        if (type != null && !type.isEmpty() && timeOfLastEntry > 0) {
                            ...
                            switch (type) {
                                case "BG":
                                   UserError.Log.d(TAG, "DATA_ITEM_RECEIVED_PATH received! Current last_send_previous=" + JoH.dateTimeText(last_send_previous));
                                   UserError.Log.i(TAG, "DATA_ITEM_RECEIVED_PATH received! Received BGs confirmed up to " + JoH.dateTimeText(timeOfLastEntry));
                                    if (timeOfLastEntry >= last_send_previous) {
                                       ...
                                       UserError.Log.d(TAG, "DATA_ITEM_RECEIVED_PATH received!  Updated last_send_previous=" + JoH.dateTimeText(last_send_previous));
                                    } else {
                                       UserError.Log.d(TAG, "DATA_ITEM_RECEIVED_PATH received! Duplicate confirmation! Ignore timeOfLastEntry=" + JoH.dateTimeText(timeOfLastEntry));
                                    }
                                    if (mPrefs.getBoolean("enable_wearG5", false)) {
                                        if (!Ob1G5CollectionService.usingNativeMode()) {
                                        ...
                                        if (dataMap != null) {
                                           UserError.Log.i(TAG, "DATA_ITEM_RECEIVED_PATH received! New Request to sync BGs from " + JoH.dateTimeText(last_send_previous));
                                    ...
                                    break;
                                case "LOG":
                                   UserError.Log.d(TAG, "DATA_ITEM_RECEIVED_PATH received! Current last_send_previous_log=" + JoH.dateTimeText(last_send_previous_log));
                                   UserError.Log.i(TAG, "DATA_ITEM_RECEIVED_PATH received! Received LOGS confirmed up to " + JoH.dateTimeText(timeOfLastEntry));
                                    if (timeOfLastEntry >= last_send_previous_log) {
                                        last_send_previous_log = timeOfLastEntry;
                                        PersistentStore.setLong(pref_last_send_previous_log, last_send_previous_log);
                                       UserError.Log.d(TAG, "DATA_ITEM_RECEIVED_PATH received!  Updated last_send_previous_log=" + JoH.dateTimeText(last_send_previous_log));
                                    } else {
                                       UserError.Log.d(TAG, "DATA_ITEM_RECEIVED_PATH received! Duplicate confirmation! Ignore timeOfLastEntry=" + JoH.dateTimeText(timeOfLastEntry));
                                    }
                                    if (mPrefs.getBoolean("sync_wear_logs", false)) {
                                        dataMap = getWearLogData(send_log_count, last_send_previous_log, (send_log_count / 3), syncLogsRequested);
                                        if (dataMap != null) {
                                           UserError.Log.i(TAG, "DATA_ITEM_RECEIVED_PATH received! New Request to sync LOGS from " + JoH.dateTimeText(last_send_previous_log));
                                            sendData(SYNC_LOGS_PATH, dataMap.toByteArray());
                                        }
                                        //Indicates this request was triggered by phone ACTION_SYNC_LOGS request; -1 indicates request was triggered by watch in doBackground()
                                           ...
                                           UserError.Log.d(TAG, "DATA_ITEM_RECEIVED_PATH received! No outstanding logs! ACTION_SYNC_LOGS request completed!! Ongoing requests syncLogsRequested=" + syncLogsRequested);
                                            sendSyncRequested(SYNC_LOGS_REQUESTED_PATH, syncLogsRequested);
                                        } else
                                           UserError.Log.d(TAG, "DATA_ITEM_RECEIVED_PATH received! No outstanding logs! SYNC_LOGS_PATH request triggered by watch doBackground.  syncLogsRequested=" + syncLogsRequested);
                                    }
                                    break;
                                case "STEP":
                                   UserError.Log.d(TAG, "DATA_ITEM_RECEIVED_PATH received! Current last_send_previous_step_sensor=" + JoH.dateTimeText(last_send_previous_step_sensor));
                                   UserError.Log.i(TAG, "DATA_ITEM_RECEIVED_PATH received! Received Steps confirmed up to " + JoH.dateTimeText(timeOfLastEntry));
                                   ...
                                   UserError.Log.d(TAG, "DATA_ITEM_RECEIVED_PATH received!  Updated last_send_previous_step_sensor=" + JoH.dateTimeText(last_send_previous_step_sensor));
                                    if (mPrefs.getBoolean("use_wear_health", false)) {
                                        ...
                                        if (dataMap != null) {
                                           UserError.Log.i(TAG, "DATA_ITEM_RECEIVED_PATH received! New Request to sync Steps from " + JoH.dateTimeText(last_send_previous_step_sensor));
                                            sendData(SYNC_STEP_SENSOR_PATH, dataMap.toByteArray());
                                        }
                                    }
                                    break;
                                case "HEART":
                                   UserError.Log.d(TAG, "DATA_ITEM_RECEIVED_PATH received! Current last_send_previous_heart_sensor=" + JoH.dateTimeText(last_send_previous_heart_sensor));
                                   UserError.Log.i(TAG, "DATA_ITEM_RECEIVED_PATH received! Received Heart confirmed up to " + JoH.dateTimeText(timeOfLastEntry));
                                   ...
                                   UserError.Log.d(TAG, "DATA_ITEM_RECEIVED_PATH received!  Updated last_send_previous_heart_sensor=" + JoH.dateTimeText(last_send_previous_heart_sensor));
                                    if (mPrefs.getBoolean("use_wear_health", false)) {
                                        dataMap = getWearHeartSensorData(send_heart_count, last_send_previous_heart_sensor, (send_heart_count / 3));
                                        if (dataMap != null) {
                                           UserError.Log.i(TAG, "DATA_ITEM_RECEIVED_PATH received! New Request to sync heart from " + JoH.dateTimeText(last_send_previous_heart_sensor));
                                            sendData(SYNC_HEART_SENSOR_PATH, dataMap.toByteArray());
                                        }
                                    }
                                    break;
                                case "TREATMENTS":
                                   UserError.Log.d(TAG, "DATA_ITEM_RECEIVED_PATH received! Current last_send_previous_treatments=" + JoH.dateTimeText(last_send_previous_treatments));
                                   UserError.Log.i(TAG, "DATA_ITEM_RECEIVED_PATH received! Received treatments confirmed up to " + JoH.dateTimeText(timeOfLastEntry));
                                    if (timeOfLastEntry >= last_send_previous_treatments) {
                                       ...
                                       UserError.Log.d(TAG, "DATA_ITEM_RECEIVED_PATH received!  Updated last_send_previous_treatments=" + JoH.dateTimeText(last_send_previous_treatments));
                                    ...
                                    if (dataMap != null) {
                                       UserError.Log.i(TAG, "DATA_ITEM_RECEIVED_PATH received! New Request to sync treatments from " + JoH.dateTimeText(last_send_previous_treatments));
                                        sendData(SYNC_TREATMENTS_PATH, dataMap.toByteArray());
                                    }
                                    break;
                                case "BM":
                                   UserError.Log.d(TAG, "Benchmark: onDataChanged received from sendDataReceived timeOfLastEntry=" + JoH.dateTimeText(timeOfLastEntry) + " Path=" + path);
                                   UserError.Log.d(TAG, "Benchmark: onDataChanged DATA_ITEM_RECEIVED_PATH msg=" + msg);
                                    break;
                            }
                        }
                    /* TODO remove once confirm not needed
    ...
    public synchronized static void createTreatment(DataMap dataMap, Context context) {
       UserError.Log.d(TAG, "createTreatment dataMap=" + dataMap);
        ...
        if (bloodtest > 0) {
           UserError.Log.d(TAG, "createTreatment bloodtest=" + bloodtest);
            BloodTest.createFromCal(bloodtest, timeoffset, "Manual Entry", treatment.uuid);
        }
        else UserError.Log.d(TAG, "createTreatment bloodtest=0 " + bloodtest);
        showTreatments(context, "all");
    ...
    public synchronized static void sendTreatment(String notes) {
       UserError.Log.d(TAG, "sendTreatment WEARABLE_TREATMENT_PAYLOAD notes=" + notes);
        DataMap dataMap = new DataMap();
    ...
    private synchronized void sendSyncRequested (String path, long syncLogsRequested) {
       ...
       UserError.Log.d(TAG, "sendSyncRequested syncLogsRequested=" + syncLogsRequested);
        DataMap dataMap = new DataMap();
    ...
    private boolean isSafeToDeleteDB() {//TODO remove once confirm not needed
        ...
        if (last_bg != null && last_send_previous <= last_bg.timestamp) {
           UserError.Log.d(TAG, "onDataChanged SYNC_DB_PATH requestData for last_send_previous < last_bg.timestamp:" + JoH.dateTimeText(last_send_previous) + "<="+ JoH.dateTimeText(last_bg.timestamp));
            requestData();
            return false;
        }
        if (mPrefs.getBoolean("sync_wear_logs", false)) {
            ...
            if (last_log != null && last_send_previous_log <= last_log.timestamp) {
               UserError.Log.d(TAG, "onDataChanged SYNC_DB_PATH requestData for last_send_previous_log < last_log.timestamp:" + JoH.dateTimeText(last_send_previous_log) + "<=" + JoH.dateTimeText((long) last_log.timestamp));
                return false;
            }
        }
        return true;
    ...
    private boolean resetDataToLatest(DataMap dataMap, Context context) {//KS
        if (dataMap != null) {
           ...
           UserError.Log.d(TAG, "resetDataToLatest dataMap.datetime=" + JoH.dateTimeText(dmTimestamp.longValue()) + " dataMap.sgvDouble=" + dataMap.getDouble("sgvDouble"));
            // todo ratelimit
            Sensor.InitDb(context);//ensure database has already been initialized
            final BgReading last = BgReading.last();
            if (last != null) {
                long bgTimestamp = last.timestamp;
               UserError.Log.d(TAG, "resetDataToLatest last.timestamp=" + JoH.dateTimeText(bgTimestamp) + " last.calculated_value=" + last.calculated_value);
    ...
    private static void dataMap(DataMap dataMap, BgReading bg, SharedPreferences sPrefs, com.eveningoutpost.dexdrip.utilitymodels.BgGraphBuilder bgGraphBuilder) {//KS
       UserError.Log.d(TAG, "dataMap bgTimestamp=" + JoH.dateTimeText(bg.timestamp) + " calculated_value=" + bg.calculated_value);
    ...
    private boolean fuzzyNodeCompare(String left, String right) {
        try {
           ...
           UserError.Log.e(TAG, "fuzzyNodeCompare NullPointerException ", e);
            return false;
        }
    }
    ...
    public boolean overrideLocale(DataMap dataMap) {
        if (mPrefs.getBoolean("overrideLocale", false)) {
            ...
            if (newLocale != null && !oldLocale.equals(newLocale)) {
                try {
                   UserError.Log.d(TAG, "overrideLocale locale from " + oldLocale + " to " + newLocale);
                    Context context = getApplicationContext();
                   ...
                   UserError.Log.d(TAG, "overrideLocale default locale " + Locale.getDefault() + " resource locale " + context.getResources().getConfiguration().locale);
                    DataMap dm = new DataMap();
                    dm.putString("locale", localeStr);
                    sendLocalMessage("locale", dm);
                    return true;
                } catch (Exception e) {
                   UserError.Log.e(TAG, "overrideLocale Exception e: " + e);
                }
            }
        }
        return false;
    ...
    private synchronized void syncPrefData(DataMap dataMap) {//KS
        ...
        try {
           UserError.Log.d(TAG, "syncPrefData dataMap=" + dataMap);
            if (localnode == null || (localnode != null && localnode.isEmpty())) setLocalNodeName();

            String dexCollector = dataMap.getString(DexCollectionType.DEX_COLLECTION_METHOD, "None");//BluetoothWixel "DexcomG5"
           UserError.Log.d(TAG, "syncPrefData dataMap dexCollector=" + dexCollector + " mPrefs DexCollectionType.DEX_COLLECTION_METHOD:" + mPrefs.getString(DexCollectionType.DEX_COLLECTION_METHOD, "xxxxxxxx"));
            DexCollectionType collectionType = DexCollectionType.getType(dexCollector);

           UserError.Log.d(TAG, "syncPrefData dataMap dexCollector:" + dexCollector + " mPrefs dex_collection_method=" + mPrefs.getString(DexCollectionType.DEX_COLLECTION_METHOD, "xxxxxxxx"));

            if (!dexCollector.equals(mPrefs.getString(DexCollectionType.DEX_COLLECTION_METHOD, "xxxxxxxx"))) {
               UserError.Log.d(TAG, "syncPrefData dexCollector:" + dexCollector + " collectionType.name=" + collectionType.name());
           ...
           UserError.Log.d(TAG, "syncPrefData is_using_bt:" + is_using_bt);
           ...
           UserError.Log.d(TAG, "syncPrefData enter enable_wearG5: " + enable_wearG5 + " force_wearG5:" + force_wearG5 + " node_wearG5:" + node_wearG5 + " prefs_node_wearG5:" + prefs_node_wearG5 + " localnode:" + localnode);

            if (!node_wearG5.equals(prefs_node_wearG5)) {
                prefs.putString("node_wearG5", node_wearG5);
               UserError.Log.d(TAG, "syncPrefData node_wearG5 pref set to dataMap:" + node_wearG5);
            }
            if (force_wearG5 && node_wearG5.equals("")) {
                prefs.putString("node_wearG5", localnode);
                node_wearG5 = localnode;
               UserError.Log.d(TAG, "syncPrefData node_wearG5 set empty string to localnode:" + localnode);
            }
            //if (!node_wearG5.equals(localnode)) {
            if (force_wearG5 && !fuzzyNodeCompare(localnode,node_wearG5)) {
               UserError.Log.d(TAG, "syncPrefData localnode != node_wearG5 disable force_wearG5 for this watch device! \nlocalnode: " + localnode + "\nnode_wearG5:" + node_wearG5);
                force_wearG5 = false;
            }

            if (force_wearG5 != mPrefs.getBoolean("force_wearG5", false)) {
               UserError.Log.d(TAG, "syncPrefData force_wearG5:" + force_wearG5);
                prefs.putBoolean("force_wearG5", force_wearG5);
            }
            if (enable_wearG5 != mPrefs.getBoolean("enable_wearG5", false)) {
               UserError.Log.d(TAG, "syncPrefData enable_wearG5:" + enable_wearG5);
                prefs.putBoolean("enable_wearG5", enable_wearG5);
            }

            String dex_txid = dataMap.getString("dex_txid", "ABCDEF");
           UserError.Log.d(TAG, "syncPrefData dataMap dex_txid=" + dex_txid);
            if (!dex_txid.equals(mPrefs.getString("dex_txid", ""))) {
               UserError.Log.d(TAG, "syncPrefData dex_txid:" + dex_txid);
                prefs.putString("dex_txid", dex_txid);
                stopBtService();//Change requires collector restart
            }

            String share_key = dataMap.getString("share_key", "SM00000000");
           UserError.Log.d(TAG, "syncPrefData dataMap share_key=" + share_key);
            if (!share_key.equals(mPrefs.getString("share_key", "SM00000000"))) {//change requires collector restart
               UserError.Log.d(TAG, "syncPrefData share_key:" + share_key);
           ...
           UserError.Log.d(TAG, "syncPrefData dataMap units=" + units);
            prefs.putString("units", units);
           UserError.Log.d(TAG, "syncPrefData prefs units=" + mPrefs.getString("units", "mgdl"));

            double high = dataMap.getDouble("high", 170.0);
            double low = dataMap.getDouble("low", 70.0);
           UserError.Log.d(TAG, "syncPrefData dataMap highMark=" + high + " highMark=" + low);
            ...
            if ((DexCollectionType.getDexCollectionType().equals(DexCollectionType.DexcomG5) ||
               ...
               UserError.Log.i(TAG, "syncPrefData wearBatteryLevel=" + wearBatteryLevel);
           ...
           UserError.Log.d(TAG, "syncPrefData exit enable_wearG5: " + enable_wearG5 + " force_wearG5:" + force_wearG5 + " node_wearG5:" + node_wearG5);

            if (!Sensor.isActive()) {
               UserError.Log.d(TAG, "syncPrefData No Active Sensor!! Request WEARABLE_INITDB_PATH before starting BT Collection Service: " + DexCollectionType.getDexCollectionType());
                sendData(WEARABLE_INITDB_PATH, null);
            }
    ...
    private boolean processConnect() {//KS
       UserError.Log.d(TAG, "processConnect enter");
        boolean enable_wearG5 = mPrefs.getBoolean("enable_wearG5", false);
        boolean force_wearG5 = mPrefs.getBoolean("force_wearG5", false);
        boolean bStarted = false;
        if (enable_wearG5) {
           UserError.Log.d(TAG, "processConnect enable_wearG5=true");
            if (!force_wearG5){
               UserError.Log.d(TAG, "processConnect force_wearG5=false - stopBtService and requestData");
                stopBtService();
                ListenerService.requestData(this);
            }
            else {
               UserError.Log.d(TAG, "processConnect force_wearG5=true - startBtService");
                bStarted = true;
                startBtService();
            }
        }
        else {
           UserError.Log.d(TAG, "processConnect enable_wearG5=false - stopBtService and requestData");
            stopBtService();
            ListenerService.requestData(this);
        }
        return bStarted;
    ...
    private void syncSensorData(DataMap dataMap, Context context) {//KS
       UserError.Log.d(TAG, "syncSensorData");
        if (dataMap != null) {
            String uuid = dataMap.getString("uuid");
           UserError.Log.d(TAG, "syncSensorData add Sensor for uuid=" + uuid);
            ...
            if (uuid != null && !uuid.isEmpty()) {
               UserError.Log.d(TAG, "syncSensorData add Sensor for uuid=" + uuid + " timestamp=" + started_at + " timeString=" +  JoH.dateTimeText(started_at));
                Sensor sensor = Sensor.getByUuid(uuid);
                if (sensor == null) {
                   UserError.Log.d(TAG, "syncSensorData createUpdate new Sensor...");
                    Sensor.createUpdate(started_at, 0, latest_battery_level, sensor_location, uuid);
                    Sensor newsensor = Sensor.currentSensor();
                    if (newsensor != null) {
                       UserError.Log.d(TAG, "syncSensorData createUpdate Sensor with uuid=" + uuid + " started at=" + started_at);
                    } else
                       UserError.Log.d(TAG, "syncSensorData Failed to createUpdate new Sensor for uuid=" + uuid);
                } else
                   UserError.Log.d(TAG, "syncSensorData Sensor already exists with uuid=" + uuid);
            }
        }
    }

    private void syncActiveBtDeviceData(DataMap dataMap, Context context) {//KS
       UserError.Log.d(TAG, "syncActiveBtDeviceData");
        if (dataMap != null) {
            String name = dataMap.getString("name", "");
            String address = dataMap.getString("address", "");
            Boolean connected = dataMap.getBoolean("connected", false);
           UserError.Log.d(TAG, "syncActiveBtDeviceData add ActiveBluetoothDevice for name=" + name + " address=" + address + " connected=" + connected);
            Sensor.InitDb(context);//ensure database has already been initialized
    ...
    private void syncAlertTypeData(DataMap dataMap, Context context) {//KS
       UserError.Log.d(TAG, "syncAlertTypeData");
        ...
        if (entries != null) {
           ...
           UserError.Log.d(TAG, "syncAlertTypeData add AlertType Table entries count=" + entries.size());
            Sensor.InitDb(context);//ensure database has already been initialized
            ...
            for (DataMap entry : entries) {
                if (entry != null) {
                    ...
                    if (alertrecord != null) {
                        ...
                        if (exists != null) {
                           UserError.Log.d(TAG, "syncAlertTypeData AlertType exists for uuid=" + data.uuid + " name=" + data.name);
                            ...
                            data.save();
                           UserError.Log.d(TAG, "syncAlertTypeData AlertType does not exist for uuid=" + data.uuid);
                        }
                        exists = AlertType.get_alert(data.uuid);
                        if (exists != null)
                           UserError.Log.d(TAG, "syncAlertTypeData AlertType GSON saved BG: " + exists.toS());
                        else
                           UserError.Log.d(TAG, "syncAlertTypeData AlertType GSON NOT saved");
                    }
                }
            }
        }
    ...
    private synchronized void syncCalibrationData(DataMap dataMap, Context context) {//KS
       UserError.Log.d(TAG, "syncCalibrationData");
        ...
        if (entries != null) {
           ...
           UserError.Log.d(TAG, "syncCalibrationData add Calibration Table entries count=" + entries.size());
            Sensor.InitDb(context);//ensure database has already been initialized
                ...
                for (DataMap entry : entries) {
                    if (entry != null) {
                        ...
                        if (calibration != null) {
                            ...
                            if (exists != null) {
                               UserError.Log.d(TAG, "syncCalibrationData Calibration exists for uuid=" + bgData.uuid + " bg=" + bgData.bg + " timestamp=" + bgData.timestamp + " timeString=" +  JoH.dateTimeText(bgData.timestamp));
                               ...
                               UserError.Log.d(TAG, "syncCalibrationData Calibration does not exist for uuid=" + bgData.uuid + " timestamp=" + bgData.timestamp + " timeString=" +  JoH.dateTimeText(bgData.timestamp));
                                //Calibration.adjustRecentBgReadings(adjustPast ? 30 : 2);
                            }
                            exists = Calibration.findByUuid(bgData.uuid);
                            if (exists != null)
                               UserError.Log.d(TAG, "syncCalibrationData Calibration GSON saved BG: " + exists.toS());
                            else
                               UserError.Log.d(TAG, "syncCalibrationData Calibration GSON NOT saved");
                        }
                    }
                }
            }
            else {
               UserError.Log.d(TAG, "syncCalibrationData No Active Sensor!! Request WEARABLE_INITDB_PATH");
                sendData(WEARABLE_INITDB_PATH, null);
            }
            if (changed) {
    ...
    private synchronized void deleteTreatment(DataMap dataMap) {
        ...
        for (String uuid : entries) {
           UserError.Log.d(TAG, "syncTreatmentsData deleteTreatment for uuid=" + uuid);
            Treatments.delete_by_uuid(uuid);
        }
    }

    private synchronized void syncTreatmentsData(DataMap dataMap, Context context) {
       UserError.Log.d(TAG, "syncTreatmentsData");

        boolean changed = false;
        String action = dataMap.getString("action");
        if (action.equals("delete")) {
           UserError.Log.d(TAG, "syncTreatmentsData Delete Treatments");
            ...
            if (entries != null) {
                Gson gson = new GsonBuilder()
                            ...
                            .serializeSpecialFloatingPointValues()
                            .create();
               UserError.Log.d(TAG, "syncTreatmentsData add Treatments Table entries count=" + entries.size());
                Sensor.InitDb(context);//ensure database has already been initialized
                for (DataMap entry : entries) {
                    if (entry != null) {
                        ...
                        if (record != null) {
                            ...
                            if (exists != null) {
                               UserError.Log.d(TAG, "syncTreatmentsData save existing Treatments for action insert uuid=" + data.uuid + " timestamp=" + data.timestamp + " timeString=" + JoH.dateTimeText(data.timestamp) + " carbs=" + data.carbs + " insulin=" + data.insulin + " exists.systime=" + JoH.dateTimeText(exists.systimestamp));
                                ...
                                data.save();
                               UserError.Log.d(TAG, "syncTreatmentsData create new treatment for action insert uuid=" + data.uuid + " timestamp=" + data.timestamp + " timeString=" + JoH.dateTimeText(data.timestamp) + " carbs=" + data.carbs + " insulin=" + data.insulin + " systime=" + JoH.dateTimeText(data.systimestamp));
                            }
                        }
                    }
                }
    ...
    private synchronized void syncBloodTestData(DataMap dataMap, Context context) {//KS
       UserError.Log.d(TAG, "syncBloodTestData");
        ...
        if (entries != null) {
           ...
           UserError.Log.d(TAG, "syncBloodTestData add BloodTest Table entries count=" + entries.size());
            Sensor.InitDb(context);//ensure database has already been initialized
            for (DataMap entry : entries) {
                if (entry != null) {
                    String record = entry.getString("data");
                    if (record != null) {
                        ...
                        if (exists != null) {
                           UserError.Log.d(TAG, "syncBloodTestData save existing BloodTest for uuid=" + data.uuid + " timestamp=" + data.timestamp + " timeString=" +  JoH.dateTimeText(data.timestamp) + " mgdl=" + data.mgdl + " state=" + data.state);
                            ...
                            changed = true;
                            data.save();
                           UserError.Log.d(TAG, "syncBloodTestData create new BloodTest for uuid=" + data.uuid + " timestamp=" + data.timestamp + " timeString=" +  JoH.dateTimeText(data.timestamp) + " mgdl=" + data.mgdl + " state=" + data.state);
                        }
                    }
                }
            }
    ...
    public static void showTreatments(Context context, String extra) {
       UserError.Log.d(TAG, "showTreatments enter");
    ...
    public static DataMap getTreatments(long startTime) {
        ...
        if (last != null) {
           UserError.Log.d(TAG, "getTreatments last.timestamp:" +  JoH.dateTimeText(last.timestamp));
        }
        List<Treatments> graph = Treatments.latestForGraph(60, startTime);
        if (!graph.isEmpty()) {
           UserError.Log.d(TAG, "getTreatments graph size=" + graph.size());
            ...
            if (entries != null) {
                entries.putDataMapArrayList("entries", dataMaps);
               UserError.Log.d(TAG, "getTreatments entries=" + entries);
            }
            return entries;
        }
        else return null;
    ...
    private static boolean includeTreatment(Treatments data) {
        if (data.carbs > 0 || data.insulin > 0)
            return true;
        else //generated by Simulation
            return data.notes != null && !data.notes.isEmpty() && !data.notes.contains("watchkeypad");
    }
    ...
    public static DataMap getCalibrations(long startTime) {
        ...
        if (last != null) {
           UserError.Log.d(TAG, "getCalibrations last.timestamp:" +  JoH.dateTimeText(last.timestamp));
        }
        ...
        if (!graph.isEmpty()) {
           UserError.Log.d(TAG, "getCalibrations graph size=" + graph.size());
            final ArrayList<DataMap> dataMaps = new ArrayList<>(graph.size());
            DataMap entries = null;
            ...
            if (entries != null) {
                entries.putDataMapArrayList("entries", dataMaps);
               UserError.Log.d(TAG, "getCalibrations entries=" + entries);
            }
            return entries;
        }
        else return null;
    ...
    public static DataMap getBloodTests(long startTime) {
        ...
        if (last != null) {
           UserError.Log.d(TAG, "getBloodTests last.timestamp:" +  JoH.dateTimeText(last.timestamp));
        }
        List<BloodTest> graph = BloodTest.latestForGraph(60, startTime);
        if (!graph.isEmpty()) {
           UserError.Log.d(TAG, "getBloodTests graph size=" + graph.size());
            final ArrayList<DataMap> dataMaps = new ArrayList<>(graph.size());
            ...
            entries.putDataMapArrayList("entries", dataMaps);
           UserError.Log.d(TAG, "getBloodTests entries=" + entries);
            return entries;
        }
        else {
           UserError.Log.d(TAG, "getBloodTests no entries for startTime=" + JoH.dateTimeText(startTime));
            return null;
        }
    }
    ...
    private synchronized void syncBgData(DataMap dataMap, Context context) {//KS
       UserError.Log.d(TAG, "syncBGData");

        boolean changed = false;
        int battery = dataMap.getInt("battery");
        ArrayList<DataMap> entries = dataMap.getDataMapArrayList("entries");
       UserError.Log.d(TAG, "syncBGData add BgReading Table battery=" + battery );
        if (entries != null) {
           ...
           UserError.Log.d(TAG, "syncBGData add BgReading Table entries count=" + entries.size());
...
*/
                            ...
                            if (exists != null) {
                               UserError.Log.d(TAG, "syncBGData BG already exists for uuid=" + bgData.uuid + " timestamp=" + bgData.timestamp + " timeString=" + JoH.dateTimeText(bgData.timestamp));
                                try {
                                   UserError.Log.d(TAG, "syncBGData exists timeString=" + JoH.dateTimeText(exists.timestamp) + "  exists.calibration.uuid=" + exists.calibration.uuid + " exists=" + exists.toS());
                                } catch (NullPointerException e) {
                                   UserError.Log.d(TAG, "" + e); // usually when calibration.uuid is null because data is from g5 native
                                }
                                ...
                                if (calibration != null) {
                                   ...
                                   UserError.Log.e(TAG, "syncBGData existing BgReading calibrationUuid not found by byuuid; calibrationUuid=" + calibrationUuid + " bgData.calibration_uuid=" + bgData.calibration_uuid + " bgData.uuid=" + bgData.uuid + " timeString=" +  JoH.dateTimeText(bgData.timestamp));
                                }
                            } else {
                                Calibration calibration = Calibration.byuuid(calibrationUuid);
                                calibration = calibration != null ? calibration : Calibration.byuuid(bgData.calibration_uuid);
                                if (calibration != null) {
                                   UserError.Log.d(TAG, "syncBGData add BG; calibration does exist for uuid=" + bgData.uuid + " timestamp=" + bgData.timestamp + " timeString=" + JoH.dateTimeText(bgData.timestamp));
                                    ...
                                    if (bgData.source_info != null && bgData.source_info.contains("Native")) {
                                       ...
                                       UserError.Log.e(TAG, "syncBGData new BgReading calibrationUuid not found by byuuid; cannot save! calibrationUuid=" + calibrationUuid + " bgData.calibration_uuid=" + bgData.calibration_uuid + " bgData.uuid=" + bgData.uuid + " timeString=" + JoH.dateTimeText(bgData.timestamp));
                                    }
                                }
                            }
                        }
               ...
               UserError.Log.d(TAG, "syncBGData No Active Sensor!! Request WEARABLE_INITDB_PATH");
                sendData(WEARABLE_INITDB_PATH, null);
            }
        }
        if (changed) {//otherwise, wait for doBackground ACTION_RESEND
           UserError.Log.d(TAG, "syncBGData BG data has changed, refresh watchface, phone battery=" + battery );
            resendData(getApplicationContext(), battery);
            CustomComplicationProviderService.refresh();
        }
        else
           UserError.Log.d(TAG, "syncBGData BG data has NOT changed, do not refresh watchface, phone battery=" + battery );
    }
    ...
    private boolean isCollectorRunning() {
        ...
        if (serviceClass != null) {
           ...
           UserError.Log.d(TAG, "DexCollectionType.getCollectorServiceClass(): " + serviceClass.getName() + " Running: " + true);
            return result;
        }
        return false;
    }
    ...
    private synchronized void startBtService() {//KS
       UserError.Log.d(TAG, "startBtService");
        if (is_using_bt) {
            if (checkLocationPermissions()) {
               UserError.Log.d(TAG, "startBtService start BT Collection Service: " + DexCollectionType.getDexCollectionType());
                //if (restartWatchDog()) {
                //    stopBtService();
                //}
                if (!isCollectorRunning()) {
                   ...
                   UserError.Log.d(TAG, "startBtService AFTER startService mLocationPermissionApproved " + mLocationPermissionApproved);
                } else {
                   UserError.Log.d(TAG, "startBtService collector already running!");
                }
            }
        }
    }
    ...
    private boolean checkLocationPermissions() {//KS
       ...
       UserError.Log.d(TAG, "checkLocationPermissions  mLocationPermissionApproved:" + mLocationPermissionApproved);
        ...
        if (is_using_bt) {//(prefs.getBoolean("aggressive_service_restart", false) || DexCollectionType.isFlakey()) {
            if (!BgReading.last_within_millis(stale_millis)) {
                if (JoH.ratelimit("aggressive-restart", aggressive_backoff_timer)) {
                   UserError.Log.e(TAG, "Aggressively restarting wear collector service due to lack of reception: backoff: "+aggressive_backoff_timer);
    ...
    private void stopBtService() {
       UserError.Log.d(TAG, "stopService call stopService");
       ...
       UserError.Log.d(TAG, "stopBtService cancel Notifications wakeIntent");
    }

    public static void requestData(Context context) {
       UserError.Log.d(TAG, "requestData (Context context) ENTER");
    ...
    private void setupStepSensor() {
        if (mPrefs.getBoolean("use_wear_health", false) || mPrefs.getBoolean("showSteps", false)) {
           UserError.Log.d(TAG, "Start Step Counter Sensor");
           ...
           UserError.Log.d(TAG, "Stop Step Counter Sensor");
    ...
    private final SensorEventListener mListener = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            if (type == SENS_STEP_COUNTER) {
               ...
               UserError.Log.d(TAG, "onSensorChanged Sensor " + type + " name = " + event.sensor.getStringType());
               UserError.Log.d(TAG, "onSensorChanged accuracy = " + event.accuracy);
               UserError.Log.d(TAG, "onSensorChanged MaxDelay = " + event.sensor.getMaxDelay());
               UserError.Log.d(TAG, "onSensorChanged t = " + t + " text = " + JoH.dateTimeText(t));
               UserError.Log.d(TAG, "onSensorChanged last_movement_timestamp = " + last_movement_timestamp + " text = " + JoH.dateTimeText(last_movement_timestamp));
               ...
               UserError.Log.d(TAG, "onSensorChanged delay = " + delay + " JoH.DateTimeText(delay) = " + JoH.dateTimeText(delay) + " (delay + (event.timestamp / 1000000L)) = " + delay + (event.timestamp / 1000000L) + " text= " + JoH.dateTimeText(delay + (event.timestamp / 1000000L)));

                PebbleMovement last = PebbleMovement.last();
                boolean sameDay = last != null && isSameDay(t, last.timestamp);
                if (!sameDay) {
                    initCounters();
                   UserError.Log.d(TAG, "onSensorChanged initCounters initCounters mSteps = " + mSteps + " mCounterSteps = " + mCounterSteps + " mPreviousCounterSteps = " + mPreviousCounterSteps + " last_movement_timestamp = " + last_movement_timestamp);
                }
                if (mCounterSteps < 1) {
                    // initial value
               ...
               UserError.Log.d(TAG, "onSensorChanged Total step count: " + mSteps + " mCounterSteps: " + mCounterSteps + " mPreviousCounterSteps: " + mPreviousCounterSteps + " event.values[0]: " + event.values[0]);

                if (last_movement_timestamp < t) {//KS BUG SW3 seems to set event.timestamp to time when sensor listener is registered
                   UserError.Log.d(TAG, "onSensorChanged Movement for mSteps: " + mSteps + " event.values[0]: " + event.values[0] +
                    ...
                    if (last_movement_timestamp == 0 || (sameDay && last != null && last.metric == mSteps)) {//skip initial movement or duplicate steps
                       UserError.Log.d(TAG, "onSensorChanged Initial sensor movement! Skip initial movement record, or duplicate record. last.metric=" + (last != null ? last.metric : "null"));
                    } else {
                        final PebbleMovement pm = PebbleMovement.createEfficientRecord(t, mSteps);//event.timestamp * 1000, (int) event.values[0]
                       UserError.Log.d(TAG, "Saving Movement: " + pm.toS());
                    }
                    last_movement_timestamp = t;
                    PersistentStore.setLong(pref_last_movement_timestamp, last_movement_timestamp);
                   UserError.Log.d(TAG, "onSensorChanged sendLocalMessage mSteps: " + mSteps + " t: " + JoH.dateTimeText(t) + " last_movement_timestamp: " + JoH.dateTimeText(last_movement_timestamp));
                    sendSensorLocalMessage(mSteps, t);
                } else {
                   UserError.Log.e(TAG, "onSensorChanged last_movement_timestamp > t! Reset last_movement_timestamp to current time.");
                    last_movement_timestamp = t;
                    PersistentStore.setLong(pref_last_movement_timestamp, last_movement_timestamp);
                }
            } else {
               UserError.Log.e(TAG,"onSensorChanged: unknown sensor type! "+type);
            }
        }
    ...
    public static boolean isSameDay(long t, long last) {
       ...
       UserError.Log.d(TAG, "isSameDay Sensor curCal.DAY_OF_MONTH=" + curCal.get(Calendar.DAY_OF_MONTH) + " lastCal.DAY_OF_MONTH=" + lastCal.get(Calendar.DAY_OF_MONTH) + " t=" + JoH.dateTimeText(t) + " last.timestamp=" + JoH.dateTimeText(last) + " " + last);
        return curCal.get(Calendar.DAY_OF_MONTH) == lastCal.get(Calendar.DAY_OF_MONTH) && curCal.get(Calendar.YEAR) == lastCal.get(Calendar.YEAR) && curCal.get(Calendar.MONTH) == lastCal.get(Calendar.MONTH);
    ...
    private synchronized void startMeasurement() {
        ...
        //}
        mCounterSteps = 0;
       UserError.Log.i(TAG, "startMeasurement SensorService Event listener for step counter sensor register");
        ...
        if (mSensorManager != null) {
            if (stepCounterSensor != null) {
                if (mPrefs.getBoolean("showSteps", false)) {
                   ...
                   UserError.Log.d(TAG, "startMeasurement delay " + delay + " seconds.");
                   UserError.Log.d(TAG, "startMeasurement Event listener for step counter sensor registered with a max delay of " + delay + " seconds.");
                    mSensorManager.registerListener(mListener, stepCounterSensor, SensorManager.SENSOR_DELAY_UI, delay * BATCH_LATENCY_1s);
                }
                else {
                   UserError.Log.d(TAG, "startMeasurement Event listener for step counter sensor registered with a max delay of " + BATCH_LATENCY_400s);
                    mSensorManager.registerListener(mListener, stepCounterSensor, SensorManager.SENSOR_DELAY_NORMAL, BATCH_LATENCY_400s);
                }
            } else {
               UserError.Log.d(TAG, "startMeasurement No Step Counter Sensor found");
            }
        }
    }

    private synchronized void stopMeasurement() {
       UserError.Log.i(TAG, "stopMeasurement");
        try {
            if (mSensorManager != null) {
               UserError.Log.i(TAG, "stopMeasurement STOP Event listener for step counter sensor register");
                mSensorManager.unregisterListener(mListener);
            }
        } catch (Exception e) {
           UserError.Log.i(TAG, "StopStepMeasurement exception: " + e);
        }
    }
    ...
    private void restartMeasurement() {
        PebbleMovement last = PebbleMovement.last();
        boolean sameDay = last != null && ListenerService.isSameDay(System.currentTimeMillis(), last.timestamp);
        if (!sameDay) {
            initCounters();
           UserError.Log.d(TAG, "restartMeasurement Sensor isSameDay=false initCounters mSteps = " + mSteps + " mCounterSteps = " + mCounterSteps + " mPreviousCounterSteps = " + mPreviousCounterSteps + " last_movement_timestamp = " + JoH.dateTimeText(last_movement_timestamp));
            stopMeasurement();
            startMeasurement();
        } else {
           UserError.Log.d(TAG, "restartMeasurement Sensor isSameDay=true PersistentStore mSteps = " + mSteps + " mCounterSteps = " + mCounterSteps + " mPreviousCounterSteps = " + mPreviousCounterSteps + " last_movement_timestamp = " + JoH.dateTimeText(last_movement_timestamp));
        }
    }
    ...
    private void resetCounters() {
       ...
       UserError.Log.d(TAG, "resetCounters Sensor Enter PersistentStore mSteps = " + mSteps + " mCounterSteps = " + mCounterSteps + " mPreviousCounterSteps = " + mPreviousCounterSteps + " last_movement_timestamp = " + JoH.dateTimeText(last_movement_timestamp));

        PebbleMovement last = PebbleMovement.last();
        boolean sameDay = last != null && ListenerService.isSameDay(System.currentTimeMillis(), last.timestamp);
        if (!sameDay) {
            initCounters();
           UserError.Log.d(TAG, "resetCounters Sensor isSameDay=false initCounters mSteps = " + mSteps + " mCounterSteps = " + mCounterSteps + " mPreviousCounterSteps = " + mPreviousCounterSteps + " last_movement_timestamp = " + JoH.dateTimeText(last_movement_timestamp));
        } else {
            mCounterSteps = 0;
            mPreviousCounterSteps = mSteps;
           UserError.Log.d(TAG, "resetCounters Sensor isSameDay=true PersistentStore mSteps = " + mSteps + " mCounterSteps = " + mCounterSteps + " mPreviousCounterSteps = " + mPreviousCounterSteps + " last_movement_timestamp = " + JoH.dateTimeText(last_movement_timestamp));
        }
    }
    ...
    private synchronized void initCounters() {
       ...
       UserError.Log.d(TAG, "initCounters Saving First Movement: " + pm.toS() + " at midnight t=" + JoH.dateTimeText(t));
        mSteps = 0;
        mCounterSteps = 0;
    ...
    private void logAvailableSensors() {
       ...
       UserError.Log.d(TAG, "=== LIST AVAILABLE SENSORS ===");
       UserError.Log.d(TAG, String.format(Locale.getDefault(), "|%-35s|%-38s|%-6s|", "SensorName", "StringType", "Type"));
        for (android.hardware.Sensor sensor : sensors) {
           UserError.Log.v(TAG, String.format(Locale.getDefault(), "|%-35s|%-38s|%-6s|", sensor.getName(), sensor.getStringType(), sensor.getType()));
        }
       UserError.Log.d(TAG, "=== LIST AVAILABLE SENSORS ===");
    }
    ...
    private void setLocalNodeName () {
        ...
        result.setResultCallback(getLocalNodeResult -> {
            if (!getLocalNodeResult.getStatus().isSuccess()) {
               UserError.Log.e(TAG, "ERROR: failed to getLocalNode Status=" + getLocalNodeResult.getStatus().getStatusMessage());
            } else {
               UserError.Log.d(TAG, "getLocalNode Status=: " + getLocalNodeResult.getStatus().getStatusMessage());
    ...
    public static void requestAPK(int position) {
       UserError.Log.d(TAG, "Requesting APK from phone: position: " + position);
    ...
    public void onCapabilityChanged(CapabilityInfo capabilityInfo) {
       ...
       UserError.Log.d(TAG, "onCapabilityChanged mPhoneNodeID:" + (phoneNode != null ? phoneNode.getId() : ""));//mPhoneNodeId
        //onPeerConnected and onPeerDisconnected deprecated at the same time as BIND_LISTENER

        if (phoneNode != null && !phoneNode.getId().isEmpty()) {
            if (JoH.ratelimit("on-connected-nodes-sync", 1200)) {
               UserError.Log.d(TAG, "onCapabilityChanged event - attempting resync");
                requestData();
            } else {
               UserError.Log.d(TAG, "onCapabilityChanged event - ratelimited");
            }
    ...
    public void onConnected(Bundle bundle) {
       UserError.Log.d(TAG, "onConnected call requestData");
    ...
    public void onDestroy() {
       ...
       UserError.Log.d(TAG, "Stop Sensors");
        stopMeasurement();


        if (mPrefs.getBoolean("enable_wearG5", true)) {
           UserError.Log.d(TAG, "Start BT Collection Service");
            stopBtService();
        }
    }
