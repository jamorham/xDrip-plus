...
public class NFCReaderX {
            ...
            if (useReaderMode) {
                try {
                    ...
                    mNfcAdapter.enableReaderMode(context, (ReaderCallback) tag -> {
                       UserError.Log.d(TAG, "Reader mode tag discovered");
        ...
        synchronized (tag_lock) {
            if (!tag_discovered) {
                ...
                if ((!last_read_succeeded) && (JoH.ratelimit("nfc-debounce", 5)) || (JoH.ratelimit("nfc-debounce", 60))) {
                    ...
                    if (d)
                       UserError.Log.d(TAG, "NFC tag discovered - going to read data");
        ...
        protected void onPostExecute(Tag tag) {
            ...
            try {
                ...
                if (succeeded) {
                   ...
                   UserError.Log.d(TAG, "Scan did not succeed so ignoring buffer");
                }
        ...
        protected Tag doInBackground(Tag... params) {
            ...
            if (read_lock.tryLock()) {
                ...
                try {
                    ...
                    if (d)UserError.Log.d(TAG, "Attempting to read tag data");
                    try {
                        //boolean connected = false;
                        try {
                            nfcvTag.connect();
                        } catch (IOException e) {
                           UserError.Log.d(TAG, "Trying second nfc connect");
                            Thread.sleep(250);
                            nfcvTag.connect();
                        }
                        ...
                        try {
                            ...
                            if ((diag != null) && (diag.length > 0)) {
                               UserError.Log.d(TAG, "Diagnostic ->: " + HexDump.dumpHexString(diag, 0, diag.length).trim() + " len: " + diag.length);
                               ...
                               UserError.Log.d(TAG, "Diagnostic <-: " + HexDump.dumpHexString(replyBlock, 0, replyBlock.length).trim() + " len: " + replyBlock.length);
                            }
                        } catch (Exception e) {
                        ...
                        if (multiblock) {
                            ...
                            for (int i = 0; i <= 43; i = i + 3) {
                                ...
                                if (d)
                                   UserError.Log.d(TAG, "Received multiblock reply, offset: " + i + " sized: " + replyBlock.length);
                                if (d)
                                   UserError.Log.d(TAG, HexDump.dumpHexString(replyBlock, 0, replyBlock.length));
                            ...
                            for (int i = 0; i < 43; i++) {
                                ...
                                if (d)
                                   UserError.Log.d(TAG, HexDump.dumpHexString(oneBlock, 0, oneBlock.length));
                       ...
                       UserError.Log.d(TAG, "GOT TAG DATA!");
                    ...
                    if (d)UserError.Log.d(TAG, "Tag data reader exiting");
                    return tag;
                } finally {
                    read_lock.unlock();
                    Home.staticBlockUI(context, false);
               ...
               UserError.Log.d(TAG, "Already read_locked! - skipping");
                return null;
            }
    ...
    public static void vibrate(Context context, int pattern) {
        ...
        if (Pref.getBooleanDefaultFalse("nfc_scan_vibrate")) {
            ...
            vibrate.cancel();
            if (d)UserError.Log.d(TAG, "About to vibrate, pattern: " + pattern);
            try {
    ...
    public static void handleHomeScreenScanPreference(Context context, boolean state) {
        try {
           UserError.Log.d(TAG, "HomeScreen Scan State: " + (state ? "enable" : "disable"));
