...
public class ReadDataShare {
    ...
    public void getRecentEGVs(final Action1<EGVRecord[]> recordListener) {
        ...
        final Action1<byte[]> fullPageListener = s -> ParsePage(read(0,s).getData(), recordType, recordListener);
        Action1<Integer> databasePageRangeCaller = s -> readDataBasePage(recordType, s, fullPageListener);
    ...
    public void getTimeSinceEGVRecord(final EGVRecord egvRecord, final Action1<Long> timeSinceEgvRecord) {
        Action1<Long> tempSystemTimeListener = s -> Observable.just(s - egvRecord.getSystemTimeSeconds()).subscribe(timeSinceEgvRecord);
        readSystemTime(tempSystemTimeListener);
    }

    public void ping(final Action1<Boolean> pingListener) {
        Action1<byte[]> pingReader = s -> Observable.just(read(0, s).getCommand() == Constants.ACK).subscribe(pingListener);
        writeCommand(Constants.PING, pingReader);
    }

    public void readBatteryLevel(final Action1<Integer> batteryLevelListener) {
        //TODO: find out if this should be wrapped in read(s).getData();
        Action1<byte[]> batteryLevelReader = s -> Observable.just(ByteBuffer.wrap(s).order(ByteOrder.LITTLE_ENDIAN).getInt()).subscribe(batteryLevelListener);
    ...
    public void readSerialNumber(final Action1<String> serialNumberListener) {
        final Action1<byte[]> manufacturingDataListener = s -> {
    ...
    public void readDisplayTime(final Action1<Date> displayTimeListener) {
        Action1<Long> tempSystemTimeListener = s -> {
            final long systemTime = s;
            Action1<Long> tempSystemTimeListener1 = s1 -> {
                Date dateDisplayTime = Utils.receiverTimeToDate(systemTime + s1);
                Observable.just(dateDisplayTime).subscribe(displayTimeListener);
            };
            readDisplayTimeOffset(tempSystemTimeListener1);
        };
        readSystemTime(tempSystemTimeListener);
    }

    public void readSystemTime(final Action1<Long> systemTimeListener) {
        Action1<byte[]> systemTimeReader = s -> Observable.just(Utils.receiverTimeToDate(ByteBuffer.wrap(read(0,s).getData()).order(ByteOrder.LITTLE_ENDIAN).getInt()).getTime()).subscribe(systemTimeListener);
    ...
    public void readDisplayTimeOffset(final Action1<Long> displayTimeOffsetListener) {
        Action1<byte[]> displayTimeOffsetReader = s -> Observable.just((long) ByteBuffer.wrap(read(0,s).getData()).order(ByteOrder.LITTLE_ENDIAN).getInt()).subscribe(displayTimeOffsetListener);
    ...
    private void readDataBasePageRange(int recordType, final Action1<Integer> databasePageRangeCaller) {
        ...
        final Action1<byte[]> databasePageRangeListener = s -> Observable.just(ByteBuffer.wrap(new ReadPacket(s).getData()).order(ByteOrder.LITTLE_ENDIAN).getInt(4)).subscribe(databasePageRangeCaller);
    ...
    private <T> T readDataBasePage(final int recordType, int page, final Action1<byte[]> fullPageListener) {
        ...
        final Action1<byte[]> databasePageReader = s -> {
           UserError.Log.d("ReadDataShare", "Database Page Reader received SIZE: " + s.length);
            byte[] temp = s;
            if (accumulatedResponse == null) {
                ...
                try {
                   ...
                   UserError.Log.d("ReadDataShare", "Combined Response length: " + accumulatedResponse.length);
