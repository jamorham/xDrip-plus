...
public class BIGChart extends WatchFace implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    void startAnimation() {
        if (d)UserError.Log.d(TAG, "CircleWatchface start startAnimation");
    ...
    public void addToWatchSet(DataMap dataMap) {
        ...
        if (d)UserError.Log.d(TAG, "addToWatchSet bgDataList.size()=" + bgDataList.size());

        ArrayList<DataMap> entries = dataMap.getDataMapArrayList("entries");
        if (entries != null) {
            if (d)UserError.Log.d(TAG, "addToWatchSet entries.size()=" + entries.size());
            for (DataMap entry : entries) {
                addDataMap(entry);
            }
        } else {
