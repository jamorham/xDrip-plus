...
public class TransmitterData extends Model {
    ...
    public static synchronized TransmitterData create(byte[] buffer, int len, Long timestamp) {
        ...
        {
           UserError.Log.e(TAG, "Got exception processing fields in protocol: " + e);
        }
        return null;
    }
    ...
    private static void saveWearBattery() {
       ...
       UserError.Log.i(TAG, "create wearBatteryLevel=" + wearBatteryLevel);
    ...
    public static void cleanup(long timestamp) {
        List<TransmitterData> transmitterData = new Select()
                ...
                .orderBy("timestamp desc")
                .execute();
        if (transmitterData != null)UserError.Log.d(TAG, "cleanup TransmitterData size=" + transmitterData.size());
    ...
    public static void updateTransmitterBatteryFromSync(final int battery_level) {
        try {
            ...
            if ((battery_level != td.sensor_battery_level) || ((JoH.ts()-td.timestamp)>(1000*60*60))) {
               ...
               UserError.Log.d(TAG,"Saving synced sensor battery, new level: "+battery_level);
                td.save();
            } else {
               UserError.Log.d(TAG,"Synced sensor battery level same as existing: "+battery_level);
            }
        } catch (Exception e) {
