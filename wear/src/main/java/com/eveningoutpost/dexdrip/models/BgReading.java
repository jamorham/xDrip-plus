...
public class BgReading extends Model implements ShareUploadableBg {
    ...
    static BgReading getForPreciseTimestamp(long timestamp, double precision, boolean lock_to_sensor) {
       ...
       UserError.Log.d(TAG, "getForPreciseTimestamp: No luck finding a BG timestamp match: " + JoH.dateTimeText(timestamp) + " precision:" + precision + " Sensor: " + ((sensor == null) ? "null" : sensor.getId()));
        return null;
    }
    ...
    public static BgReading create(double raw_data, double filtered_data, Context context, Long timestamp, boolean quick) {
        ...
        if (calibration == null) {
           ...
           UserError.Log.d(TAG, "Calibrations, so doing everything: " + calibration.uuid);
    ...
    public static BgReading createFromRawNoSave(Sensor sensor, Calibration calibration, double raw_data, double filtered_data, long timestamp) {
        ...
        if (calibration.check_in) {
            ...
            if (lastBgReading != null && lastBgReading.calibration != null) {
               UserError.Log.d(TAG, "Create calibration.uuid=" + calibration.uuid + " bgReading.uuid: " + bgReading.uuid + " lastBgReading.calibration_uuid: " + lastBgReading.calibration_uuid + " lastBgReading.calibration.uuid: " + lastBgReading.calibration.uuid);
               UserError.Log.d(TAG, "Create lastBgReading.calibration_flag=" + lastBgReading.calibration_flag + " bgReading.timestamp: " + bgReading.timestamp + " lastBgReading.timestamp: " + lastBgReading.timestamp + " lastBgReading.calibration.timestamp: " + lastBgReading.calibration.timestamp);
               UserError.Log.d(TAG, "Create lastBgReading.calibration_flag=" + lastBgReading.calibration_flag + " bgReading.timestamp: " + JoH.dateTimeText(bgReading.timestamp) + " lastBgReading.timestamp: " + JoH.dateTimeText(lastBgReading.timestamp) + " lastBgReading.calibration.timestamp: " + JoH.dateTimeText(lastBgReading.calibration.timestamp));
    ...
    public static String getSlopeArrowSymbolBeforeCalibration() {
        ...
        if ((last!=null) && (last.size()==2)) {
            ...
            if (time_delta<=(BgGraphBuilder.DEXCOM_PERIOD * 2)) {
               ...
               UserError.Log.d(TAG, "SlopeArrowBeforeCalibration: guess delta: " + estimated_delta + " delta2: " + estimated_delta2 + " timedelta: " + time_delta);
        ...
        if (bgr != null) {
            try {
                if (readingNearTimeStamp(bgr.timestamp) == null) {
                   ...
                   UserError.Log.d(TAG, "Ignoring duplicate bgr record due to timestamp: " + json);
                }
            } catch (Exception e) {
        ...
        if (bgr != null) {
            ...
            try {
                if (readingNearTimeStamp(bgr.timestamp) == null) {
                   ...
                   UserError.Log.d(TAG, "Ignoring duplicate bgr record due to timestamp: " + timestamp);
                }
            } catch (Exception e) {
               UserError.Log.d(TAG, "Could not save BGR: " + e.toString());
            }
    ...
    public static void cleanup(long timestamp) {
        try {
            ...
            List<BgReading> data = new Select()
                    ...
                    .orderBy("timestamp desc")
                    .execute();
            if (data != null)UserError.Log.d(TAG, "cleanup BgReading size=" + data.size());
    ...
    public static List<BgReading> getXRecentPoints(int NumReadings) {
        ...
        if (new Date().getTime() - latest.get(NumReadings - 1).timestamp > (NumReadings * 5 + 6) * 60 * 1000) {
           UserError.Log.d(TAG_ALERT, "getXRecentPoints we don't have enough points from the last " + (NumReadings * 5 + 6) + " minutes, returning null");
            return null;
        }
        return latest;
                        ...
                        if (high_since == 0) {
                           ...
                           UserError.Log.d(TAG, "Registering start of persistent high at time now");
                            ...
                            if (high_for_mins > threshold_mins) {
                               ...
                               UserError.Log.d(TAG, "Persistent high below time threshold at: " + high_for_mins);
                            }
                        }
                    }
                } else {
    ...
    public static void checkForRisingAllert(Context context) {
       ...
       UserError.Log.d(TAG_ALERT, "checkForRisingAllert will check for rate of " + friseRate);
    ...
    private static boolean checkForDropRiseAllert(float MaxSpeed, boolean drop) {
       UserError.Log.d(TAG_ALERT, "checkForDropRiseAllert called drop=" + drop);
       ...
       UserError.Log.d(TAG_ALERT, "checkForDropRiseAllert returning true speed is " + (bg_diff3 / time3));
        return true;
    }
    ...
    */
    ...
    private Boolean Unclear() {
       UserError.Log.d(TAG_ALERT, "Unclear filtered_data=" + filtered_data + " raw_data=" + raw_data);
    ...
    private static Long getUnclearTimeHelper(List<BgReading> latest, Long interstingTime, final Long now) {
                ...
                if (LastGoodTime == 0L) {
                   UserError.Log.d(TAG_ALERT, "We are starting a good period at "+ bgReading.timestamp);
                    ...
                    if(LastGoodTime - bgReading.timestamp >= MAX_INFLUANCE) {
                       ...
                       UserError.Log.d(TAG_ALERT, "We have a good period from " + bgReading.timestamp + " to " + LastGoodTime + "returning " + (now - UnclearTime +5 *60000));
       ...
       UserError.Log.d(TAG_ALERT, "We scanned all over, but could not find a good period. we have a bad value, so assuming that the whole period is bad" +
    ...
    static void TestgetUnclearTime(String input, Long interstingTime, Long expectedResult) {
        ...
        if (result >= expectedResult * 60000 - 20000 && result <= expectedResult * 60000+20000) {
           ...
           UserError.Log.d(TAG_ALERT, "Test failed expectedResult = " + expectedResult + " result = "+ result / 60000.0);
        }
