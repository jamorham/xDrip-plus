...
//  @Expose
  ...
  public String sensor_location;
    ...
    public static Sensor create(long started_at) {
       ...
       UserError.Log.d("SENSOR MODEL:", sensor.toString());
        return sensor;
    }
    ...
    public static Sensor create(long started_at, String uuid) {//KS
        ...
        {
           UserError.Log.d("SENSOR create new error ", uuid);
            return null;
        }
    }
    ...
    public static void createUpdate(long started_at, long stopped_at,  int latest_battery_level, String uuid) {
        ...
        if (sensor != null) {
           UserError.Log.d("SENSOR", "updating an existing sensor");
        } else {
           UserError.Log.d("SENSOR", "creating a new sensor");
            sensor = new Sensor();
        }
        sensor.started_at = started_at;
    ...
    public static void stopSensor() {
       ...
       UserError.Log.i("SENSOR", "Sensor stopped at " + sensor.stopped_at);
        sensor.save();
        SensorSendQueue.addToQueue(sensor);
    ...
    public String toS() {//KS
        Gson gson = new GsonBuilder()
                ...
                .create();
       UserError.Log.d("SENSOR", "Sensor toS uuid=" + this.uuid + " started_at=" + this.started_at + " active=" + this.isActive() + " battery=" + this.latest_battery_level + " location=" + this.sensor_location + " stopped_at=" + this.stopped_at);
    ...
    public static void DeleteAndInitDb(Context context) {//KS
        ...
        try {
           ...
           UserError.Log.d("wearSENSOR", "DeleteAndInitDb DexDrip.db deleted and initialized.");
        } catch (Exception e) {
           UserError.Log.e("wearSENSOR", "DeleteAndInitDb CATCH Error.");
        }
    }
    ...
    public static void InitDb(Context context) {//KS
        ...
        try {
            ...
            if (db != null) {
               UserError.Log.d("wearSENSOR", "InitDb DB exists");
            }
            else {
                ActiveAndroid.initialize(dbConfiguration);
               UserError.Log.d("wearSENSOR", "InitDb DB does NOT exist. Call ActiveAndroid.initialize()");
            }
        } catch (Exception e) {
            ActiveAndroid.initialize(dbConfiguration);
           UserError.Log.d("wearSENSOR", "InitDb CATCH: DB does NOT exist. Call ActiveAndroid.initialize()");
        }
    }
    ...
    public static boolean TableExists(String table) {//KS
        try {
            ...
            if (db != null) {
               ...
               UserError.Log.d("wearSENSOR", "TableExists table does NOT exist:" + table);
                return true;
            }
            else {
               UserError.Log.d("wearSENSOR", "TableExists Cache.openDatabase() failed.");
                return false;
            }
        } catch (Exception e) {
           UserError.Log.d("wearSENSOR", "TableExists CATCH error table:" + table);
            return false;
        }
    }
    ...
    public static Sensor getByUuid(String xDrip_sensor_uuid) {
        if(xDrip_sensor_uuid == null) {
           UserError.Log.d("wearSENSOR", "getByUuid xDrip_sensor_uuid is null");
            return null;
        }
       UserError.Log.d("wearSENSOR", "getByUuid xDrip_sensor_uuid is " + xDrip_sensor_uuid);
        ...
        //if (TableExists("Sensor")) {//com.eveningoutpost.dexdrip.Models.Sensor
            try {//KS
               ...
               UserError.Log.d("wearSENSOR", "getByUuid CATCH Select error xDrip_sensor_uuid is " + xDrip_sensor_uuid);
                return null;
            }
        //}
        //else return null;
    ...
    public static void updateBatteryLevel(int sensorBatteryLevel, boolean from_sync) {
        ...
        if (sensor == null)
        {
           UserError.Log.d("Sensor","Cant sync battery level from master as sensor data is null");
            return;
        }
    ...
    public static void updateSensorLocation(String sensor_location) {
        Sensor sensor = currentSensor();
        if (sensor == null) {
           UserError.Log.e("SENSOR MODEL:", "updateSensorLocation called but sensor is null");
            return;
        }
        sensor.sensor_location = sensor_location;
        sensor.save();
