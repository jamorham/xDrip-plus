...
public class ActiveBgAlert extends Model {
    ...
    public static ActiveBgAlert getOnly() {
        try {
           ...
           UserError.Log.d(TAG,"ActiveBgAlert rebuilding table strcuture");
            fixUpTable();
            return null;
        }
    }
    ...
    public static AlertType alertTypegetOnly() {
        ...
        if(alert == null) {
           UserError.Log.d(TAG, "alertTypegetOnly did not find the active alert as part of existing alerts. returning null");
    ...
    public static void Create(String alert_uuid, boolean is_snoozed, Long next_alert_at) {
       UserError.Log.d(TAG, "ActiveBgAlert Create called");
        fixUpTable();
        ActiveBgAlert aba = getOnly();
        if (aba == null) {
    ...
    public static void ClearData() {
       UserError.Log.d(TAG, "ActiveBgAlert ClearData called");
        ActiveBgAlert aba = getOnly();
        if (aba != null) {
            aba.delete();
        }
    ...
    public static void ClearIfSnoozeFinished() {
       UserError.Log.d(TAG, "ActiveBgAlert ClearIfSnoozeFinished called");
        ActiveBgAlert aba = getOnly();
        if (aba != null) {
            if(new Date().getTime() > aba.next_alert_at) {
               UserError.Log.d(TAG, "ActiveBgAlert ClearIfSnoozeFinished deleting allert");
                aba.delete();
            }
        }
    }
