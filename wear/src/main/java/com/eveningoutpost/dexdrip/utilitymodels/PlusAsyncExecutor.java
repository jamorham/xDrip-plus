...
public class PlusAsyncExecutor implements Executor {
    ...
    public synchronized void execute(@NonNull final Runnable r) {
        ...
        if (!taskQueues.containsKey(queueId)) {
           UserError.Log.d(TAG, "New task queue for: " + queueId);
        ...
        if (qsize < 20) {
            ...
            taskQueues.get(queueId).offer(() -> {
                ...
                try {
                    ...
                    if (locksnow > 1)
                       UserError.Log.d(TAG, queueId + " Acquire Wakelocks total: " + locksnow);
                    r.run();
                } finally {
                    ...
                    if (locksnow != 0)
                       UserError.Log.d(TAG, queueId + " Release Wakelocks total: " + locksnow);
                }
            });
    ...
    private synchronized void next(String queueId) {
        ...
        if (task != null) {
           UserError.Log.d(TAG, " New thread: " + queueId);
            final Thread t = (new Thread(task));
            t.setPriority(NORM_PRIORITY - 1);
            t.start();
        } else {
           UserError.Log.d(TAG, "Queue empty: " + queueId);
        }
