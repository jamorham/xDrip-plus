...
public class SerialInputOutputManager implements Runnable {
    ...
    public synchronized void stop() {
        if (getState() == State.RUNNING) {
            UserError.Log.i(TAG, "Stop requested");
    ...
    public void run() {
        ...
        UserError.Log.i(TAG, "Running ..");
        try {
            while (true) {
                if (getState() != State.RUNNING) {
                    UserError.Log.i(TAG, "Stopping mState=" + getState());
                    break;
                }
                step();
            }
        } catch (Exception e) {
            UserError.Log.w(TAG, "Run ending due to exception: " + e.getMessage(), e);
    ...
    private void step() throws IOException {
        ...
        if (len > 0) {
            if (DEBUG) UserError.Log.i(TAG, "Read data len=" + len);
            final Listener listener = getListener();
            if (listener != null) {
        ...
        if (outBuff != null) {
            if (DEBUG) {
                UserError.Log.i(TAG, "Writing data len=" + len);
            }
