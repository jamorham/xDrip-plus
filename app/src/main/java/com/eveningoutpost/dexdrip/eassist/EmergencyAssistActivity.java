...
import static com.eveningoutpost.dexdrip.eassist.EmergencyAssist.*;
...
public class EmergencyAssistActivity extends BaseActivity {
	...
	private com.eveningoutpost.dexdrip.databinding.ActivityEmergencyAssistBinding binding;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		binding = com.eveningoutpost.dexdrip.databinding.ActivityEmergencyAssistBinding.inflate(getLayoutInflater());
	...
	private boolean checkContactsPermission() {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
			if (ContextCompat.checkSelfPermission(getApplicationContext(), Manifest.permission.READ_CONTACTS) != PackageManager.PERMISSION_GRANTED) {
				...
				JoH.show_ok_dialog(activity, gs(R.string.please_allow_permission), gs(R.string.need_contacts_permission_to_select_message_recipients), () -> ActivityCompat.requestPermissions(activity, new String[]{Manifest.permission.READ_CONTACTS}, MY_PERMISSIONS_REQUEST_CONTACTS));
				return false;
			}
		}
		return true;
	...
	private static boolean isSMSPermissionGranted() {
		return (ContextCompat.checkSelfPermission(xdrip.getAppContext(), Manifest.permission.SEND_SMS) == PackageManager.PERMISSION_GRANTED);
	}
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
			if (!isSMSPermissionGranted()) {
				if (JoH.ratelimit("check-sms-permission", 2)) {
					final AppCompatActivity activity = this;
					JoH.show_ok_dialog(activity, gs(R.string.please_allow_permission), "Need SMS permission to send text messages to your emergency contacts." + "\n\n" + "Warning this can cost money at normal telecoms rates!", () -> ActivityCompat.requestPermissions(activity, new String[]{Manifest.permission.SEND_SMS}, MY_PERMISSIONS_REQUEST_SMS));
				}
				return false;
			}
		} else {
	...
	public void masterEnable() {
		Inevitable.task("ea-master-enable", 100, () -> {
			if (prefs.getbool(EMERGENCY_ASSIST_PREF)) {
	...
	private boolean atLeastOneContact() {
		final boolean result = !binding.getContactModel().items.isEmpty();
		if (!result) {
	...
	public void askRemove(EmergencyContact contact) {
		GenericConfirmDialog.show(this, "Remove?", "Remove " + contact.name + " from emergency text message receivers list?", () -> {
