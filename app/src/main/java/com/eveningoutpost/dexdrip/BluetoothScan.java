...
import static com.eveningoutpost.dexdrip.cgm.medtrum.Medtrum.*;
...
public class BluetoothScan extends ListActivityWithMenu {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//PP
		ListView listViewBT = (ListView) findViewById(R.id.listViewBT);
		listViewBT.setOnItemClickListener((parent, view, position, id) -> {
			Log.d(TAG, "Item Clicked");
			final BluetoothDevice device = mLeDeviceListAdapter.getDevice(position);
			if (device == null || device.getName() == null) return;
			Toast.makeText(BluetoothScan.this, R.string.connecting_to_device, Toast.LENGTH_LONG).show();
			...
			synchronized (ActiveBluetoothDevice.table_lock) {
				...
				Home.startWatchUpdaterService(BluetoothScan.this, WatchUpdaterService.ACTION_SYNC_ACTIVEBTDEVICE, TAG);
			}
			...
			try {
				if (device.getName().toLowerCase().contains("limitter")
						...
						&& ((new String(adverts.get(device.getAddress()), StandardCharsets.UTF_8).contains("eLeR"))
						|| (new String(adverts.get(device.getAddress()), StandardCharsets.UTF_8).contains("data"))))) {
				...
				if (device.getName().toLowerCase().contains("xbridge")
						&& (adverts.containsKey(device.getAddress()) && (new String(adverts.get(device.getAddress()), StandardCharsets.UTF_8).contains("rfduino")))) {
				...
				if (device.getName().toLowerCase().contains("dexcom")) {
					...
					Blukon.doPinDialog(BluetoothScan.this, (Runnable) () -> {
						if (!CollectionServiceStarter.isLimitter()) {
							prefs.edit().putString("dex_collection_method", "LimiTTer").apply();
						}
						returnToHome();
		...
		listViewBT.setAdapter(mLeDeviceListAdapter);
	}
				...
				mHandler.postDelayed(() -> {
					...
					if (bluetooth_adapter != null && bluetooth_adapter.isEnabled()) {
						try {
							...
							Log.e(TAG, "error stopping scan: " + e.toString());
						}
					}
					invalidateOptionsMenu();
				}, SCAN_PERIOD);
...
//        UserError.Log.i(TAG, "Item Clicked");
//        final BluetoothDevice device = mLeDeviceListAdapter.getDevice(position);
//        if (device == null || device.getName() == null) return;
...
//            } else if (device.getName().matches("^BLU[0-9][0-9][0-9][0-9][0-9].*$")) {
//
//                Blukon.doPinDialog(this, (Runnable) () -> {
//                    if (!CollectionServiceStarter.isLimitter()) {
//                        prefs.edit().putString("dex_collection_method", "LimiTTer").apply();
//                    }
//                    returnToHome();
...
//
//        } catch (NullPointerException e) {
//            UserError.Log.i(TAG, "Got exception in listitemclick: " + Arrays.toString(e.getStackTrace()));
//        }
//    }
	...
	private void requestSerialNumber(final SharedPreferences prefs) {
		...
		dialog.findViewById(R.id.scannerButton).setOnClickListener(v -> {
			...
			if (device != null) {
				...
				if (adverts.containsKey(device.getAddress())) {
					...
					if (Pref.getBooleanDefaultFalse("engineering_mode")) {
						viewHolder.deviceAddress.append("   " + new String(adverts.get(device.getAddress()), StandardCharsets.UTF_8));
					}
				}
			}
			return view;
