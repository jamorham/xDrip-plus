...
import me.tatarka.bindingcollectionadapter2.collections.*;
...
public class EventLogActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final com.eveningoutpost.dexdrip.databinding.ActivityEventLogBinding binding = com.eveningoutpost.dexdrip.databinding.ActivityEventLogBinding.inflate(getLayoutInflater());
	...
	private void pushSearch(String query, boolean submit) {
		if (searchItem != null && searchView != null) {
			searchItem.expandActionView(); //	MenuItemCompat.expandActionView(searchItem);
	...
	public synchronized void uploadEventLogs(View v) {
		...
		builder.append("The following logs will be sent to the developers: \n\nPlease also include your email address or we will not know who they are from!\n\nFilter: ").append(model.allSeveritiesEnabled() ? "ALL" : model.whichSeveritiesEnabled()).append(!model.getCurrentFilter().equals("") ? " Search: " + model.getCurrentFilter() : "").append("\n\n");
	...
	public class ViewModel implements View.OnTouchListener {
		...
		public final MergeObservableList<UserError> items = new MergeObservableList<UserError>().insertList(streamed_items).insertList(initial_items).insertList(older_items);
...
public final ItemBinding<UserError> itemBinding = ItemBinding.of(BR.item, R.layout.item_event_log);
		...
		String whichSeveritiesEnabled() {
			...
			for (int severity : severitiesList) {
				if (severity(severity)) result.append(severity).append(" ");
			}
		...
		void filter(final String filter) {
			currentFilter = filter.toLowerCase().trim(); //filter.or(getCurrentFilter()).toLowerCase().trim();
			visible.clear();
			synchronized (items) {
...
//					|| (item.message.containsIgnoreCaseF(currentFilter)));
	...
	public class SimpleOnScaleGestureListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
		...
		public SimpleOnScaleGestureListener(ViewModel model) {
	...
	public class EventLogViewAdapterChain<T> extends BindingRecyclerViewAdapter<T> {
		...
		@Override
		public void onBindBinding(@NonNull ViewDataBinding binding, int bindingVariable, @LayoutRes int layoutId, int position, T item) {
