...
import static com.eveningoutpost.dexdrip.services.SyncService.*;
            ...
            try {
                ...
                for (int i = 0; i < processedBaseURIs.size(); i++) {
                    try {
                        ...
                        l.add(new StatusItem(processedBaseURInames.get(i), status.getString("name") + " " + status.getString("version"), status.getString("version").startsWith("0.8") ? StatusItem.Highlight.CRITICAL : StatusItem.Highlight.NORMAL, "long-press", () -> refreshStatus(store_marker)));

                        if (status.has("careportalEnabled") && !status.getBoolean("careportalEnabled")) {
                            l.add(new StatusItem("Config error in Nightscout at " + processedBaseURInames.get(i), "You must enable the careportal plugin or treatment sync will be broken", StatusItem.Highlight.BAD, "long-press", () -> refreshStatus(store_marker)));
                        }
                        ...
                        if ((dextended == null) || !dextended.has("advanced") || !dextended.getBoolean("advanced"))
                            l.add(new StatusItem("Config error in Nightscout at " + processedBaseURInames.get(i), "You must set DEVICESTATUS_ADVANCED env item to True for multiple battery status to work properly", StatusItem.Highlight.BAD, "long-press", () -> refreshStatus(store_marker)));
