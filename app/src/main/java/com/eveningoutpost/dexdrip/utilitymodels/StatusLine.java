...
public class StatusLine {
    ...
    private static String extraStatusLineReal() {
        ...
        if (Pref.getBoolean("status_line_calibration_long", false) && ((lastCalibration = Calibration.lastValid()) != null)) {
            append(sb, "slope = ");
            sb.append(String.format(Locale.getDefault(), "%.2f", lastCalibration.slope));
            sb.append(' ');
            sb.append("inter = ");
            sb.append(String.format(Locale.getDefault(), "%.2f", lastCalibration.intercept));
        }
        ...
        if (Pref.getBoolean("extra_status_calibration_plugin", false)) {
            ...
            if (plugin != null) {
                ...
                if (pcalibration != null)
                    sb.append("(").append(plugin.getAlgorithmName()).append(") s:").append(JoH.qs(pcalibration.slope, 2)).append(" i:").append(JoH.qs(pcalibration.intercept, 2));
                BgReading bgReading = BgReading.last();
                if (bgReading != null) {
                    final boolean doMgdl = Pref.getString("units", "mgdl").equals("mgdl");
                    sb.append(" \u21D2 ").append(BgGraphBuilder.unitized_string(plugin.getGlucoseFromSensorValue(bgReading.age_adjusted_raw_value), doMgdl)).append(" ").append(BgGraphBuilder.unit(doMgdl));
                }
            }
            ...
            if (Pref.getBooleanDefaultFalse("display_glucose_from_plugin") || Pref.getBooleanDefaultFalse("use_pluggable_alg_as_primary")) {
                ...
                if (plugin_xdrip != null) {
                    ...
                    if (sb.length() > 0)
                        sb.append("\n"); // not tested on the widget yet
                    if (pcalibration != null)
                        sb.append("(").append(plugin_xdrip.getAlgorithmName()).append(") s:").append(JoH.qs(pcalibration.slope, 2)).append(" i:").append(JoH.qs(pcalibration.intercept, 2));
                    BgReading bgReading = BgReading.last();
                    if (bgReading != null) {
                        final boolean doMgdl = Pref.getString("units", "mgdl").equals("mgdl");
                        sb.append(" \u21D2 ").append(BgGraphBuilder.unitized_string(plugin_xdrip.getGlucoseFromSensorValue(bgReading.age_adjusted_raw_value), doMgdl)).append(" ").append(BgGraphBuilder.unit(doMgdl));
                    }
                }
            }
