...
import com.eveningoutpost.dexdrip.evaluators.*;
...
// TODO REFACTOR
    private void FileBasedNotifications(Context context) {
        ...
        UserError.Log.i(TAG, "FileBasedNotifications called bgReading.calculated_value = " + bgReading.calculated_value + " calculated value: "+calculated_value);
        ...
        if (((sensor != null) || (Home.get_follower())) && calculated_value != 0) {
            ...
            if (newAlert == null) {
                UserError.Log.i(TAG, "FileBasedNotifications - No active notifcation exists, stopping all alerts");
            ...
            if (activeBgAlert == null) {
                UserError.Log.i(TAG, "FileBasedNotifications we have a new alert, starting to play it... " + newAlert.name);
            ...
            if (activeBgAlert.uuid.equals(newAlert.uuid)) {
                ...
                if(prefs.getBoolean("disable_alerts_stale_data", false)) {
                    ...
                    if ((new Date().getTime()) - (60000 * minutes) - BgReading.lastNoSenssor().timestamp > 0) {
                        UserError.Log.i(TAG, "FileBasedNotifications : active alert found but not replaying it because more than three readings missed :  " + newAlert.name);
                        return;
                    }
                }

                UserError.Log.i(TAG, "FileBasedNotifications we have found an active alert, checking if we need to play it " + newAlert.name);
            ...
            if (alertSnoozeOver) {
                UserError.Log.i(TAG, "FileBasedNotifications we had two alerts, the snoozed one is over, we fall down to deleting the snoozed and staring the new");
                ...
                AlertType newHigherAlert = AlertType.HigherAlert(activeBgAlert, newAlert);
                    if ((newHigherAlert == activeBgAlert)) {
                        ...
                        UserError.Log.i(TAG, "FileBasedNotifications The new alert has the same direcotion, it is lower than the one snoozed, not playing it." +
            ...
            UserError.Log.i(TAG, "Found a new alert, that is higher than the previous one will play it. " + newAlert.name);
            AlertPlayer.getPlayer().stopAlert(context, true, false);
            boolean trendingToAlertEnd = trendingToAlertEnd(context, true, newAlert);
            AlertPlayer.getPlayer().startAlert(context, trendingToAlertEnd, newAlert, EditAlertActivity.unitsConvert2Disp(doMgdl, calculated_value));
        } else {
 ...
 */
    ...
    private boolean notificationSetter(Context context) {
        ...
        if (prefs.getLong("alerts_disabled_until", 0) > new Date().getTime()) {
            UserError.Log.i("NOTIFICATIONS", "Notifications are currently disabled!!");
            return false;
        }
        ...
        UserError.Log.i(TAG, "forced_wear=" + forced_wear + " bg_notifications_watch=" + bg_notifications_watch + " persistent_high_alert_enabled_watch=" + bg_persistent_high_alert_enabled_watch);
        ...
        if (calibration_notifications) {
            ...
            try {
                ...
                UserError.Log.i(TAG,"Calibration reminder seconds: "+calibration_reminder_secs);
            } catch (Exception e) {
                UserError.Log.wtf(TAG,"Could not parse calibration_reminder_hours");
            }

            // TODO this should only clear double calibration once after calibrations are achieved
            if (bgReadings.size() >= 3) {
                if (calibrations.isEmpty() && (new Date().getTime() - bgReadings.get(2).timestamp <= (60000 * 30)) && sensor != null) {
            ...
            if (calibrations.size() >= 1 && (Math.abs(JoH.msSince(calibrations.get(0).timestamp)) > (calibration_reminder_secs * 1000))
                ...
                UserError.Log.i("NOTIFICATIONS", "Calibration difference in hours: " + ((new Date().getTime() - calibrations.get(0).timestamp)) / (1000 * 60 * 60));
                if ((!PowerStateReceiver.is_power_connected()) || (Pref.getBooleanDefaultFalse("calibration_alerts_while_charging"))) {
    ...
    private long calcuatleArmTimeUnclearalert(Context ctx, long now, boolean unclearAlert) {
        ...
        if( wakeTimeUnclear == Long.MAX_VALUE) {
            // Should not happen
            UserError.Log.e(TAG ,"calcuatleArmTimeUnclearalert wakeTimeUnclear bad value setting it to one minute from now " + new Date(wakeTimeUnclear) + " in " +  ((wakeTimeUnclear - now)/60000d) + " minutes" );
            return now + 60 * 1000;
        }
        UserError.Log.w(TAG ,"calcuatleArmTimeUnclearalert returning " + new Date(wakeTimeUnclear) + " in " +  ((wakeTimeUnclear - now)/60000d) + " minutes" );
    ...
    private long calcuatleArmTimeBg(long now) {
        ...
        if (activeBgAlert != null) {
            ...
            if (alert != null) {
                ...
                UserError.Log.i(TAG , "ArmTimer BG alert -waking at: "+ new Date(wakeTimeBg) +" in " +  (wakeTimeBg - now)/60000d + " minutes");
                if (wakeTimeBg < now) {
        ...
        UserError.Log.i("Notifications" , "calcuatleArmTimeBg returning: "+ new Date(wakeTimeBg) +" in " +  (wakeTimeBg - now)/60000d + " minutes");
        return wakeTimeBg;
    }
 ...
 // This is the absolute time, not time from now.
    private long calcuatleArmTime(Context ctx, long now, boolean unclearAlert) {
        ...
        UserError.Log.i("Notifications" , "calcuatleArmTime returning: "+ new Date(wakeTime) +" in " +  (wakeTime - now)/60000d + " minutes");
        return wakeTime;
  ...
  */
    ...
    private synchronized void scheduleWakeup(Context context, boolean unclearAlert) {
       // Calendar calendar = Calendar.getInstance();
        ...
        // TODO use JoH wakeup
        UserError.Log.i("Notifications" , "ArmTimer waking at: "+ new Date(wakeTime ) +" in " +
   ...
   /* private Notification createExtensionPage(long hours) {
        ...
        try {
            mHandler.post(() -> {
                try {
                    ...
                    UserError.Log.e(TAG, "Got runtime exception in bgOngoingNotification runnable: ", e);
    ...
    private void reportNoiseChanges()
    {
        ...
        {
            UserError.Log.uel("Noise","Changed from: "+last_noise_string+" to "+this_noise_string);
    ...
    private void calibrationNotificationCreate(String title, String content, Intent intent, int notificationId) {
        ...
        mBuilder.setVisibility(Pref.getBooleanDefaultFalse("public_notifications") ? NotificationCompat.VISIBILITY_PUBLIC : Notification.VISIBILITY_PRIVATE);
        mBuilder.setVibrate(vibratePattern);
        mBuilder.setLights(0xff00ff00, 300, 1000);
        if(calibration_override_silent) {
            mBuilder.setSound(Uri.parse(calibration_notification_sound), AudioManager.STREAM_ALARM /*AudioAttributes.USAGE_ALARM*/);
        } else {
    ...
    private NotificationCompat.Builder notificationBuilder(String title, String content, Intent intent, String channelId) {
        return new NotificationCompat.Builder(mContext, channelId)
                .setVisibility(Pref.getBooleanDefaultFalse("public_notifications") ? NotificationCompat.VISIBILITY_PUBLIC : Notification.VISIBILITY_PRIVATE)
                .setSmallIcon(R.drawable.ic_action_communication_invert_colors_on)
    ...
    public static void lowPredictAlert(Context context, boolean on, String msg) {
        ...
        if (on) {
            if ((Pref.getLong("alerts_disabled_until", 0) < JoH.tsl()) && (Pref.getLong("low_alerts_disabled_until", 0) < JoH.tsl())) {
                ...
                UserError.Log.ueh(TAG, "Not Low predict alerting due to snooze: " + msg);
            }
        } else {
    ...
    public static void persistentHighAlert(Context context, boolean on, String msg) {
        ...
        if (on) {
            if ((Pref.getLong("alerts_disabled_until", 0) < JoH.tsl()) && (Pref.getLong("high_alerts_disabled_until", 0) < JoH.tsl())) {
                ...
                UserError.Log.ueh(TAG, "Not persistent high alerting due to snooze: " + msg);
            }
        } else {
            NotificationManager mNotifyMgr = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
    ...
    private static void OtherAlert(Context context, String type, String message, int notificatioId, String channelId, boolean addDeleteIntent, long reraiseSec) {
        ...
        UserError.Log.i(TAG,"OtherAlert called " + type + " " + message + " reraiseSec = " + reraiseSec);
        ...
        if ((userNotification == null) || userNotification.timestamp <= new Date().getTime() ) {
            ...
            UserError.Log.i(TAG,"OtherAlert forced_wear localOnly=" + localOnly);
            Intent intent = new Intent(context, Home.class);
            @SuppressLint("InlinedApi") NotificationCompat.Builder mBuilder =
            ...
            if (AlertPlayer.notSilencedDueToCall()) {
                if (otherAlertsOverrideSilent && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    mBuilder.setSound(Uri.parse(otherAlertsSound), AudioManager.STREAM_ALARM /*AudioAttributes.USAGE_ALARM*/);
                } else {
            ...
            //mNotifyMgr.cancel(notificatioId);
            //Log.d(TAG, "Notify");
            UserError.Log.ueh("Other Alert",message);
