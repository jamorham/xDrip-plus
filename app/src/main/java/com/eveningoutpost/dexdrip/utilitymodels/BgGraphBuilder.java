...
import com.eveningoutpost.dexdrip.store.*;
import com.eveningoutpost.dexdrip.ui.classifier.*;
...
public class BgGraphBuilder {
    ...
    private final List<PointValue> pluginValues = new ArrayList<>();
    private final List<PointValue> rawInterpretedValues = new ArrayList<>();
    private final List<PointValue> filteredValues = new ArrayList<>();
    ...
    private final List<PointValue> iobValues = new ArrayList<>();
    private final List<PointValue> cobValues = new ArrayList<>();
    private final List<PointValue> predictedBgValues = new ArrayList<>();
    private final List<PointValue> polyBgValues = new ArrayList<>();
    private final List<PointValue> noisePolyBgValues = new ArrayList<>();
    private final List<PointValue> activityValues = new ArrayList<>();
    ...
    public synchronized List<Line> defaultLines(boolean simple) {
        ...
        try {
            ...
            // iob line
            lines.addAll(autoSplitLine(treatments[2], 10));
...
//            predictive_end_time = (new Date().getTime() + (60000 * 10) + (1000 * 60 * 60 * predictivehours)) / FUZZER; // used first in ideal/highline
    ...
    public Line avg1Line() {
        List<PointValue> myLineValues = new ArrayList<>();
    ...
    public Line predictiveHighLine() {
        List<PointValue> predictiveHighLineValues = new ArrayList<>();
...
*/
    ...
    public Viewport advanceViewport(Chart chart, Chart previewChart, float hours) {
        ...
        viewport.inset(((86400000 / hours) / FUZZER), 0);
        ...
        public synchronized void onValueSelected(int i, int i1, PointValue pointValue) {
            ...
            switch (type) {
                case PointValueExtended.BloodTest:
                    ...
                    final View.OnClickListener mBtOnClickListener = v -> Home.startHomeWithExtra(xdrip.getAppContext(), Home.BLOOD_TEST_ACTION, time.toString(), fuuid);
                    Home.snackBar(R.string.blood_test, message, mBtOnClickListener, callerActivity);
                    break;
                default:
                    final View.OnClickListener mOnClickListener = v -> Home.startHomeWithExtra(xdrip.getAppContext(), Home.CREATE_TREATMENT_NOTE, time.toString(), Double.toString(ypos));
