...
import static com.eveningoutpost.dexdrip.ui.helpers.UiHelper.*;
...
public class SourceWizard {
	...
	private static final String TAG = SourceWizard.class.getSimpleName();
	...
	public void getTreeDialog(Tree<Item> branch) {
		...
		if (!branch.data.isCollectionType()) {
			...
			final com.eveningoutpost.dexdrip.databinding.DialogTreeSelectorBinding binding = com.eveningoutpost.dexdrip.databinding.DialogTreeSelectorBinding.inflate(activity.getLayoutInflater());
			...
			builder.setPositiveButton(R.string.yes, (OnClickListener) (dialog, which) -> {
			...
			builder.setNegativeButton(R.string.no, (OnClickListener) (dialog, which) -> getTreeDialog(fbranch.getParent()));
