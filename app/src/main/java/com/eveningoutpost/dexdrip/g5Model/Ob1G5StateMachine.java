...
import static com.eveningoutpost.dexdrip.services.G5BaseService.*;
import static com.eveningoutpost.dexdrip.services.Ob1G5CollectionService.G6_SCALING;
import static com.eveningoutpost.dexdrip.services.Ob1G5CollectionService.*;
...
public class Ob1G5StateMachine {
    ...
    public static boolean doCheckAuth(Ob1G5CollectionService parent, RxBleConnection connection) {
        ...
        connection.setupNotification(Authentication)
                ...
                .doOnNext(notificationObservable -> connection.writeCharacteristic(Authentication, nn(authRequest.byteSequence))
                        .subscribe(
                                characteristicValue -> {
                                    ...
                                    connection.readCharacteristic(Authentication).subscribe(
                                            readValue -> authenticationProcessor(parent, connection, readValue), throwable -> UserError.Log.e(TAG, "Could not read after AuthRequestTX: " + throwable));
                                            ...
                                            bondRequestValue -> {
                                                ...
                                                connection.readCharacteristic(Authentication)
                                                        ...
                                                        .subscribe(
                                                                ...
                                                                }, throwable -> UserError.Log.e(TAG, "Throwable when reading characteristic after keepalive: " + throwable));
                    ...
                    switch (data_packet.type) {
                            ...
                            try {
                                if ((getVersionDetails) && (!haveFirmwareDetails())) {
                                    connection.writeCharacteristic(Control, nn(new VersionRequestTxMessage().byteSequence))
                                            .subscribe(versionValue -> UserError.Log.d(TAG, "Wrote version request"), throwable -> UserError.Log.e(TAG, "Failed to write VersionRequestTxMessage: " + throwable));
                        ...
                        case VersionRequestRxMessage:
                            ...
                            if (JoH.ratelimit("g6-evaluate", 600)) {
                                Inevitable.task("evaluteG6Settings", 10000, Ob1G5StateMachine::evaluateG6Settings);
                            }
                            break;
    ...
    private static void monitorBackFill(Ob1G5CollectionService parent, RxBleConnection connection) {
        ...
        connection.setupNotification(ProbablyBackfill)
                ...
                .subscribe(bytes -> {
                        ...
                        }, throwable -> UserError.Log.d(TAG, "backfill throwable: " + throwable)
                );
