...
import static com.eveningoutpost.dexdrip.g5Model.CalibrationState.*;
import static com.eveningoutpost.dexdrip.g5Model.Ob1G5StateMachine.*;
import static com.eveningoutpost.dexdrip.services.Ob1G5CollectionService.STATE.*;
...
public class Ob1G5CollectionService extends G5BaseService {
    ...
    public static boolean isG5WarmingUp() {
        loadCalibrationStateAsRequired();
        return lastSensorState == CalibrationState.WarmingUp && usingNativeMode();
    }
 ...
 */
    ...
    public static List<StatusItem> megaStatus() {
        ...
        if (getBatteryStatusNow) {
            l.add(new StatusItem("Battery Status Request Queued", "Will attempt to read battery status on next sensor reading", NOTICE, "long-press", () -> getBatteryStatusNow = false));
        }
        ...
        if ((bt != null) && (last_battery_query > 0)) {
            l.add(new StatusItem("Battery Last queried", JoH.niceTimeSince(last_battery_query) + " " + "ago", NORMAL, "long-press", () -> getBatteryStatusNow = true));
            if (vr != null) {
    ...
    public final SharedPreferences.OnSharedPreferenceChangeListener prefListener = (prefs, key) -> checkPreferenceKey(key, prefs);
