...
public class MongoWrapper {
    ...
    public DBCollection openMongoDb() throws UnknownHostException {
     ...
     public List<TransmitterRawData> ReadFromMongo(int numberOfRecords) {
    	...
    	List<TransmitterRawData> trd_list = new LinkedList<>();
      	DBCollection coll;
      	...
      	try {
            ...
            try {
                while(cursor.hasNext() && trd_list.size() < numberOfRecords) {
                    ...
                    UserError.Log.i(TAG, "Read an object from mongodb");
     ...
     public List<LibreWifiData> ReadFromMongoLibre(int numberOfRecords) {
        ...
        List<LibreWifiData> trd_list = new LinkedList<>();
        DBCollection coll;
        ...
        try {
            ...
            try {
                while(cursor.hasNext() && trd_list.size() < numberOfRecords) {
                    //System.out.println(cursor.next());
                    UserError.Log.i(TAG, "Read a libre object from mongodb");
                    ...
                    if(!LibreWifiReader.almostEquals(lastTrd, trd)) {
                        ...
                        System.out.println( trd.toString());
                         ...
                         UserError.Log.e(TAG, "Error, read a value from mongo, but it seems duplicate" + trd.toString());
                    }
                }
             } finally {
                cursor.close();
