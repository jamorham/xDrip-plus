    ...
    */
    ...
    public static byte[] decodeblueReaderPacket(byte[] buffer, int len) {
        ...
        if (bufferstring.startsWith("not ready for") ) { //delete the trans_failed, because its normal only if the bluereader could not read the sensor.
            ...
            UserError.Log.i(TAG, bufferstring);
            ...
            UserError.Log.i (TAG, "blueReader was set to wakeup-mode manually...");
            return null;
        } else if (bufferstring.startsWith("ECHO")) {
            UserError.Log.i (TAG, "blueReader was set to Echo-Mode manually...");
            return null;
        } else if (bufferstring.startsWith("NFC READY")) {
            UserError.Log.i (TAG, "blueReader notice that NFC is active...");
            return null;
        } else if (bufferstring.startsWith("NFC_DISABLED")) {
            UserError.Log.i (TAG, "blueReader notice that NFC is now hibernated...");
            return null;
            ...
            UserError.Log.i (TAG, "blueReader sends an unknown reaction: '" + bufferstring + "'");
            return null;
            ...
            if (Pref.getBooleanDefaultFalse("blueReader_turn_off")) {
                if (Pref.getInt("blueReader_turn_off_value",5) > Pref.getInt("bridge_battery",100)) {
                    UserError.Log.w (TAG, "blueReader will be turn off, as the battery is lower then " + Pref.getInt("blueReader_turn_off_value",5) +"%");
    ...
    private static synchronized void processNewTransmitterData(TransmitterData transmitterData, long timestamp) {
        ...
        //TODO compare with test-formular, and new Data of batterylog
    ...
    private static void writeLog(String logFile, String logLine) {
        ...
        try {
            ...
            UserError.Log.w(TAG, "log write error: " + ioe.toString());
        } finally {
            if (pWriter != null){
                pWriter.flush();
                pWriter.close();
