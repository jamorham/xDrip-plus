...
//        CalRecord secondCalRecord = calRecords[calRecords.length - 1];
            ...
            for (int i = 0; i < firstCalRecord.getCalSubrecords().length - 1; i++) {
                if (((firstCalRecord.getCalSubrecords()[i] != null && Calibration.is_new(firstCalRecord.getCalSubrecords()[i], addativeOffset))) || (i == 0 && override)) {
                    ...
                    if (calibration.timestamp > new Date().getTime()) {
                        UserError.Log.i(TAG, "ERROR - Calibration timestamp is from the future, wont save!");
                        return;
                    }
    ...
    public static boolean is_new(CalSubrecord calSubrecord, long addativeOffset) {
        ...
        if (calibration != null && Math.abs(calibration.timestamp - (calSubrecord.getDateEntered().getTime() + addativeOffset)) < (4 * 60 * 1000)) {
            UserError.Log.i("CAL CHECK IN ", "Already have that calibration!");
            return false;
        } else {
            UserError.Log.i("CAL CHECK IN ", "Looks like a new calibration!");
            return true;
        }
    }
    ...
    public static Calibration create(double bg, long timeoffset, Context context, boolean note_only, long estimatedInterstitialLagSeconds) {
        ...
        if ((bg < 40) || (bg > 400)) {
            UserError.Log.wtf(TAG, "Invalid out of range calibration glucose mg/dl value of: " + bg);
        ...
        if (sensor != null) {
            ...
            if (bgReading != null) {
                if (SensorSanity.isRawValueSane(bgReading.raw_data, DexCollectionType.getDexCollectionType(), true)) {
                    ...
                    if ((!is_follower) && (!note_only)) {
                        ...
                        UserError.Log.i(TAG, "Follower mode or note so not processing calibration deeply");
                    }
                } else {
            ...
            UserError.Log.i("CALIBRATION", "No sensor, cant save!");
        }
    ...
    private synchronized static void calculate_w_l_s(boolean extended) {
        if (Sensor.isActive()) {
            ...
            if (calibrations.size() <= 1) {
                ...
                UserError.Log.i(TAG, "Calibration slope debug: slope:" + calibration.slope + " q:" + q + " m:" + m + " p:" + p + " d:" + d);
                if ((calibrations.size() == 2 && calibration.slope < sParams.getLowSlope1()) || (calibration.slope < sParams.getLowSlope2())) { // I have not seen a case where a value below 7.5 proved to be accurate but we should keep an eye on this
                    UserError.Log.i(TAG, "calibration.slope 1 : " + calibration.slope);
                    calibration.slope = calibration.slopeOOBHandler(0);
                    UserError.Log.i(TAG, "calibration.slope 2 : " + calibration.slope);
                ...
                if ((calibrations.size() == 2 && calibration.slope > sParams.getHighSlope1()) || (calibration.slope > sParams.getHighSlope2())) {
                    UserError.Log.i(TAG, "calibration.slope 3 : " + calibration.slope);
                    calibration.slope = calibration.slopeOOBHandler(1);
                    UserError.Log.i(TAG, "calibration.slope 4 : " + calibration.slope);
                    if (calibrations.size() > 2) {
                        calibration.possible_bad = true;
                    }
                    calibration.intercept = calibration.bg - (calibration.estimate_raw_at_time_of_calibration * calibration.slope);
                ...
                UserError.Log.i(TAG, "Calculated Calibration Slope: " + calibration.slope);
                UserError.Log.i(TAG, "Calculated Calibration intercept: " + calibration.intercept);
            ...
            UserError.Log.i(TAG, "NO Current active sensor found!!");
        }
    }
        ...
        if (status == 0) {
            if (calibrations.size() == 3) {
                if ((Math.abs(thisCalibration.bg - thisCalibration.estimate_bg_at_time_of_calibration) < 30)
                        ...
                        && (calibrations.get(1).possible_bad != null && calibrations.get(1).possible_bad)) {
    ...
    private double calculateWeight() {
        ...
        UserError.Log.i(TAG, "CALIBRATIONS TIME PERCENTAGE WEIGHT: " + time_percentage);
    ...
    public static void adjustRecentBgReadings(int adjustCount) {
        ...
        if (calibrations.size() >= 3) {
            ...
            try {
                ...
                for (BgReading bgReading : bgReadings) {
                    if (bgReading.calibration != null) {
                        ...
                        UserError.Log.i(TAG, "History Rewrite: Ignoring BgReading without calibration from: " + JoH.dateTimeText(bgReading.timestamp));
                    }
                }
            ...
            try {
                ...
                UserError.Log.wtf(TAG, "Null pointer in AdjustRecentReadings ==2: " + e);
            }
        }
    ...
    public static void clearLastCalibration() {
        CalibrationRequest.clearAll();
        UserError.Log.i(TAG, "Trying to clear last calibration");
    ...
    public static void clearCalibrationByUUID(String uuid) {
        ...
        if (calibration != null) {
            CalibrationRequest.clearAll();
            UserError.Log.i(TAG, "Trying to clear last calibration: " + uuid);
            calibration.invalidate();
            CalibrationSendQueue.addToQueue(calibration, xdrip.getAppContext());
            newFingerStickData();
        } else {
            UserError.Log.i(TAG,"Could not find calibration to clear: "+uuid);
        }
    }
    ...
    public static void upsertFromMaster(Calibration jsonCalibration) {
        ...
        try {
            ...
            if (sensor == null) {
                UserError.Log.e(TAG, "No sensor found, ignoring cailbration " + jsonCalibration.sensor_uuid);
                return;
            }
            Calibration existingCalibration = byuuid(jsonCalibration.uuid);
            if (existingCalibration == null) {
                UserError.Log.i(TAG, "saving new calibration record. sensor uuid =" + jsonCalibration.sensor_uuid + " calibration uuid = " + jsonCalibration.uuid);
                jsonCalibration.sensor = sensor;
                jsonCalibration.save();
            } else {
                UserError.Log.i(TAG, "updating existing calibration record: " + jsonCalibration.uuid);
    ...
    public boolean isNote() {
	    ...
	    return (calibration.slope == 0) && (calibration.slope_confidence == note_only_marker) && (calibration.sensor_confidence == 0) && (calibration.intercept == 0);
    }

    public boolean isValid() {
        Calibration calibration = this;
	    return (calibration.slope_confidence != 0) && (calibration.sensor_confidence != 0) && (calibration.slope != 0) && (calibration.intercept != 0);
    }
