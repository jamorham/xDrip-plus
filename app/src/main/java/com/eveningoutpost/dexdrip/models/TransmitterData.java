    ...
    public static synchronized TransmitterData create(byte[] buffer, int len, Long timestamp) {
        ...
        try {
            if ((buffer[0] == 0x11 || buffer[0] == 0x15) && buffer[1] == 0x00) {
                ...
                UserError.Log.i(TAG, "create Processing a Dexbridge packet");
                ...
                UserError.Log.i(TAG, "Created transmitterData record with Raw value of " + transmitterData.raw_data + " and Filtered value of " + transmitterData.filtered_data + " at " + timestamp + " with timestamp " + transmitterData.timestamp);
            } else { //this is NOT a dexbridge packet.  Process accordingly.
                UserError.Log.i(TAG, "create Processing a BTWixel or IPWixel packet");
                        ...
                        if (data.length > 3) {
                            if ((DexCollectionType.getDexCollectionType() == DexCollectionType.LimiTTer)
                                ...
                                try {
                                    ...
                                    final int sensorAge = Integer.parseInt(data[3]);
                                    if ((sensorAge > 0) && (sensorAge < 200000))
                                        Pref.setInt("nfc_sensor_age", sensorAge);
                                } catch (Exception e) {
                                    UserError.Log.e(TAG, "Got exception processing field 4 in classic limitter protocol: " + e);
                                }
                            }
                        }
                    }
            ...
            if (lastTransmitterData != null && lastTransmitterData.timestamp >= transmitterData.timestamp) {
                UserError.Log.e(TAG, "Rejecting TransmitterData constraint: last: " + JoH.dateTimeText(lastTransmitterData.timestamp) + " >= this: " + JoH.dateTimeText(transmitterData.timestamp));
                return null;
            }
            if (lastTransmitterData != null && lastTransmitterData.raw_data == transmitterData.raw_data && Math.abs(lastTransmitterData.timestamp - transmitterData.timestamp) < (Constants.MINUTE_IN_MS * 2)) {
                UserError.Log.e(TAG, "Rejecting identical TransmitterData constraint: last: " + JoH.dateTimeText(lastTransmitterData.timestamp) + " due to 2 minute rule this: " + JoH.dateTimeText(transmitterData.timestamp));
                return null;
            }
            ...
            if (lastCalibration != null && lastCalibration.timestamp > transmitterData.timestamp) {
                UserError.Log.e(TAG, "Rejecting historical TransmitterData constraint: calib: " + JoH.dateTimeText(lastCalibration.timestamp) + " > this: " + JoH.dateTimeText(transmitterData.timestamp));
                return null;
            }
            ...
            UserError.Log.e(TAG, "Got exception processing fields in protocol: " + e + " " + HexDump.dumpHexString(buffer));
        }
        return null;
    }
    ...
    public static void updateTransmitterBatteryFromSync(final int battery_level) {
        try {
            ...
            if ((td == null) || (td.raw_data!=0))
            {
                ...
                UserError.Log.i(TAG,"Created new fake transmitter data record for battery sync");
                if (td==null) return;
            }
            if ((battery_level != td.sensor_battery_level) || ((JoH.ts()-td.timestamp)>(1000*60*60))) {
                ...
                UserError.Log.i(TAG,"Saving synced sensor battery, new level: "+battery_level);
                td.save();
            } else {
                UserError.Log.i(TAG,"Synced sensor battery level same as existing: "+battery_level);
            }
        } catch (Exception e) {
