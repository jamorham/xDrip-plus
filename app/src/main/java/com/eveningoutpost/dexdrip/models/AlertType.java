...
public class AlertType extends Model {
    ...
    public static AlertType get_highest_active_alert(Context context, double bg) {
        ...
        if (at != null) {
            UserError.Log.i(TAG_ALERT, "get_highest_active_alert_helper returned alert uuid = " + at.uuid + " alert name = " + at.name);
        } else {
            UserError.Log.i(TAG_ALERT, "get_highest_active_alert_helper returned NULL");
        }
        return at;
    }
    ...
    private static AlertType filter_alert_on_stale(AlertType alert, SharedPreferences prefs)
    {
        ...
        if (prefs.getBoolean("disable_alerts_stale_data", false)) {
            ...
            if (!BgReading.last_within_minutes(stale_minutes)) {
                UserError.Log.w(TAG, "Blocking alarm raise as data older than: " + stale_minutes);
                return null; // block
            }
        }
        return alert; // allow
    ...
    public static void print_all() {
        ...
        UserError.Log.i(TAG,"List of all alerts");
        for (AlertType alert : Alerts) {
    ...
    public static void testAll(Context context) {
        ...
        UserError.Log.i(TAG, "a1 = " + a1.toString());
        AlertType a2 = get_highest_active_alert(context, 210);
        UserError.Log.i(TAG, "a2 = " + a2.toString());


        AlertType a3 = get_alert(a1.uuid);
        UserError.Log.i(TAG, "a1 == a3 ? need to see true " + (a1==a3) + a1 + " " + a3);
        ...
        UserError.Log.i(TAG, "al1 should be null  " + al1);
        al1 = get_highest_active_alert(context, 80);
        UserError.Log.i(TAG, "al1 = " + al1.toString());
        AlertType al2 = get_highest_active_alert(context, 50);
        UserError.Log.i(TAG, "al2 = " + al2.toString());

        UserError.Log.i(TAG, "HigherAlert(a1, a2) = a1?" +  (HigherAlert(a1,a2) == a2));
        UserError.Log.i(TAG, "HigherAlert(al1, al2) = al1?" +  (HigherAlert(al1,al2) == al2));
        UserError.Log.i(TAG, "HigherAlert(a1, al1) = al1?" +  (HigherAlert(a1,al1) == al1));
        UserError.Log.i(TAG, "HigherAlert(al1, a2) = al1?" +  (HigherAlert(al1,a2) == al1));
    ...
    static public boolean  s_in_time_frame(boolean s_all_day, int s_start_time_minutes, int s_end_time_minutes) {
        ...
        UserError.Log.i(TAG, "time_now is " + time_now + " minutes" + " start_time " + s_start_time_minutes + " end_time " + s_end_time_minutes);
...
//            UserError.Log.e(TAG, "beyond_threshold returning true " );
            return true;
        } else return !above && bg <= threshold;
    }
...
//        UserError.Log.e(TAG, "should_alarm called active =  " + active );
 ...
 // return the hours part of the time
    ...
    public static boolean toSettings(Context context) {
        ...
        UserError.Log.e(TAG, "Created the string " + output);
        prefs.edit().putString("saved_alerts", output).apply(); // always leave this as commit
    ...
    public static boolean fromSettings(Context context) {
        ...
        UserError.Log.i(TAG, "read alerts string " + savedAlerts);
        ...
        UserError.Log.i(TAG, "read successfuly " + newAlerts.length);
        ...
        try {
            for (AlertType alert : newAlerts) {
                UserError.Log.e(TAG, "Saving alert " + alert.name);
                alert.save();
            }
        } catch (NullPointerException e) {
            UserError.Log.e(TAG, "Got null pointer exception 2: " + e);
        }
