...
public class DesertSync extends PlusModel {
                ...
                if (items != null) {
                    ...
                    for (final DesertSync item : items) {
                        if (item.timestamp > highestPullTimeStamp) {
                            ...
                            Inevitable.task("desert-sync-timestamp", 500, () -> UserError.Log.d(TAG, "Synced up till: " + JoH.dateTimeText(highestPullTimeStamp)));
                        }
                        processItem(item);
                    }
    ...
    private static void correctWebServiceSettings() {
        ...
        if (Pref.getString(PREF_WEBSERVICE_SECRET, "").isEmpty()) {
